...
To do this, `r Biocpkg("BASiCS")` compares the gene-specific over-dispersion
parameters between two conditions. These parameters are independent of technical noise 
...
<!---
Two case studies exemplify the use of `r Biocpkg("BASiCS")` for non-UMI and
UMI scRNA-seq data. In the first case, `r Biocpkg("BASiCS")` can be used to detect highly
and lowly variable genes and to obtain robust, gene-specific estimates to assess
...
of `r Biocpkg("BASiCS")` to test for changes in mean expression and expression
variability.In the second case, we use droplet-based scRNA-seq data to detect more subtle 
...
Finally, we provide a step-by-step case study using naive CD4^+^ T cells 
[@Martinez-jimenez2017].
<!-- and samples collected during embryonic somitogenesis [@Ibarra-Soria2018] -->
These examples were chosen to illustate the usage of 
...
4. [C1 Fluidigm data](#Tcells): We present an end-to-end analysis workflow to 
  ...
  where `r Biocpkg("BASiCS")` is used to normalise data and to detect highly and
  lowly   variable genes, and a [two group case](#Tcells-two), for which we perform 
  differential mean expression and differential variability testing.

5. [10X Genomics data](#droplet-data): We highlight the use of `r Biocpkg("BASiCS")`
for   scRNA-seq data that do not contain technical spike-in transcript. 
...
The focus of this article is to use scRNA-seq data to quantify the
magnitude of cell-to-cell expression heterogeneity within seemingly homogeneous 
cell populations. 
...
```{r overview, out.width=out_width, out.height=out_height, fig.cap = 'Graphical overview for the scRNA-seq analysis workflow described in this manuscript. Starting from a SingleCellExperiment object, we use the scater and scran Bioconductor packages to perform quality control and initial exploratory analyses. The primary focus of this workflow is to robustly quantify transcriptional heterogeneity within seemingly homogeneous cell populations. For this purpose, we apply the BASiCS Bioconductor package, illustrating how BASiCS can be used to analyse a single or multiple pre-specified groups of cells.', echo=FALSE}
...
We use the `r Biocpkg("SingleCellExperiment")` package to convert an input
...
technical spike-in molecules (these can be accessed via the `altExp()` 
function).
...
The `r Biocpkg("SimpleSingleCell")` workflow and 
[*OSCA*](https://osca.bioconductor.org/) online book provides an extensive
overview on important aspects of how to perform QC of scRNA-seq data, including
exploratory analyses [@Lun2016;@Amezquita2019].
...
sources (see `modelGeneVar`). 
Moreover, the `modelGeneCV2` function can be used to infer an overall trend 
between gene-specific mean and squared coefficent of variation (CV^2^) 
estimates. 
...
We also load `r CRANpkg("ggplot2")` to aid in visualising the cell- and gene- 
...
be assessed using `plot`, `BASiCS_EffectiveSize`, `BASiCS_DiagHist` and 
`BASiCS_DiagPlot`. The `plot` function provides
...
If the user decides to set `Regression = FALSE`, `r Biocpkg("BASiCS")` will not
estimate the regression trend, and will not supply the residual over-dispersion 
...
To use `coda` functions, the individual chains returned by `r Biocpkg("BASiCS")` need to
be transformed into a MCMC object that `coda` recognises using the `coda::mcmc` 
function. `r Biocpkg("BASiCS")` also offers a number of functions to visualise and assess
the convergence of MCMC chains. In particular, we will use 
