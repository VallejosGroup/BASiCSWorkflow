...
Subsequently, we describe a step-by-step workflow that uses
...
Moreover, lowly expressed genes that represent less reliable information is 
...
It should be noted that normalisation
is not strictly necessary in the `r Biocpkg("BASiCS")` workflow.
Count-based models for dimensionality reduction and can be used throughout,
removing the need for log normalisation [@Townes2019;@Lopez2018].
...
QC metrics such as those shown in Figure \@ref(fig:gene-selection).
...
# while also ensure the order is identical
...
Practical guidance about diagnostics for MCMC convergence and sample size
...
The output from `BASiCS_MCMC` is a `BASiCS_Chain` object that contains the MCMC 
...
### Diagnostics for BASiCS

<!--- THIS SECTION IS WIP ---> 

Before we perform downstream analyses, it is critical to assess the convergence 
of the MCMC algorithm.
An instance of MCMC has converged when it has reached 
its stationary distribution. Thus, if convergence has been achieved, the mean
of each parameter should not evolve significantly over time, and
draws are expected to be stochastic fluctuations around a horizontal trend.
However, it is not possible to prove convergence; at most it is possible to 
prove a lack of convergence. To assess convergence,
multiple graphical and quantitative convergence diagnostics have been proposed 
(e.g. [@CowlesCarlin1996; @BrooksGelman1998]). Some advocate the use of 
multiple MCMC chains using different starting values in order to ensure that
the algorithm consistently converges to the same distribution.
For `r Biocpkg("BASiCS")`, we have observed that by using informed starting
values and a sufficient value for `N` and `Burn`, convergence is consistent.
However, it is still important to evaluate quantitative measures of chain
convergence [@Heidelberger1981;@Geweke1995].

As well as assessing the convergence of the MCMC algorithm, it is important
to ensure that the MCMC algorithm has efficiently sampled from the posterior
distribution. `r Biocpkg("BASiCS")` provides a number of graphical functions 
to assess the efficiency of the MCMC algorithm. One such output can be
obtained using the `plot` method for `BASiCS_Chain` objects.
The output of this function contains three pieces of information: traceplots,
marginal distributions, and the autocorrelation function.

Traceplots can be used to 
visually assess the history of posterior draws generated by the algorithm for a
specific parameter (e.g. Figure \@ref(fig:convergence-naive), left panel). 
As mentioned, if the algorithm has converged, this is expected to only show 
stochastic fluctuations around a horizontal trend. Departures from this 
...
be increased. It is also helpful to perform more stringent quality control
on the input data, given that genes with very low counts often suffer from slow
...
Strong autocorrelation between subsequent MCMC draws does not invalidate the
parameter values estimated, but does reduce the information content of the 
draws. Strong autocorrelation indicates that neighbouring MCMC
samples are highly dependent.
This does not affect the validity of inference on the posterior if the sampler
has time to explore the posterior distribution. It does, however, reduce the 
efficiency of the sampler.
Low sampling efficiency may mean that the samples produced
do not contain sufficient information to produce accurate estimates of the 
posterior distribution.
In other words, highly correlated MCMC samplers require more samples to produce
the same level of Monte Carlo error for an estimate. Monte Carlo error is 
defined as the variance of a Monte Carlo estimate across repetitions
[@Koehler2009].
...
plot(chain_naive, Param = "mu", Gene = 6142)
```


As `r Biocpkg("BASiCS")` infers thousands of parameters, it is 
impractical to assess graphical diagnostics, as described previously, for each 
parameter. Thus, it is helpful to use numerical diagnostics that can be applied 
to many parameters simultaneously to judge whether the algorithm has failed
to converge, or failed to sample efficiently.
For the former case, we focus here on the diagnostic criterion proposed by 
Geweke [@Geweke1995]. This produces a Z-score comparing a portion of the draws 
from the beginning of the chain (0.1 by default) to a portion of the draws
from the end of the chain (0.5 by default). Large absolute Z-scores indicate
a large difference in mean between the beginning and end of the chain, 
indicating that the algorithm has not converged by the end of the burn-in
period. As seen in Figure \@ref(fig:geweke-diag-hist), most genes have small
absolute values for this diagnostic criterion.
...
```{r geweke-diag-hist, fig.cap = "Histogram of Geweke diagnostic for mean expression across all genes in naive cells. Large absolute values for this diagnostic indicate that the sampler may not have reached the stationary distribution by the end of the burn-in period."}
library("coda")
library("ggplot2")

gew_mu_naive <- geweke.diag(mcmc(displayChainBASiCS(chain_naive, Param = "mu")))$z
which(abs(gew_mu_naive) == max(abs(gew_mu_naive)) )
ess_naive <- BASiCS_EffectiveSize(chain_naive, Param = "mu")

hist(
  gew_mu_naive,
  breaks = "FD",
  main = NULL,
  xlab = "Geweke diagnostic"
)
abline(v = c(-3, 3), col = "firebrick", lty = "dashed")
## AOC: not sure what this is meant to show?
# hist(2*pnorm(-abs(gew_mu_naive)))
```


```{r geweke-diag-mean, fig.cap="Geweke diagnostic criterion for mean expression plotted against mean expression in naive cells."}
ggplot() +
  aes(
  ...
  geom_hex() +
  labs(x = "Mean expression", y = "Geweke diagnostic\nfor mean expression") +
  geom_hline(yintercept = c(-3, 3), col = "firebrick", linetype = "dashed") +
  scale_fill_viridis()
```

```{r geweke-diag-detected, fig.cap="Geweke diagnostic criterion for mean expression plotted against the detection rate for all genes in naive cells."}
ggplot() +
  aes(
    rowData(sce_naive)$detected[rownames(rowData(sce_naive)) %in% rownames(chain_naive)], 
    gew_mu_naive
  ) +
  geom_hex() +
  labs(x = "Number of detected genes", y = "Geweke diagnostic\nfor mean expression") +
  geom_hline(yintercept = c(-3, 3), col = "firebrick", linetype = "dashed") +
  scale_fill_viridis()
```  


Given that high levels of autocorrelation reduce the efficiency of an MCMC 
sampler, it is helpful to evaluate the 
effective number of independent samples that an MCMC chain has produced.
This allows us to ascertain whether this effect has impaired our ability to
perform inferences using the results of the chain.
...
number of samples generated divided by the total autocorrelation.
...
As `r Biocpkg("BASiCS")` has thousands of parameters, it is impractical to 
explore convergence diagnostics for each of them individually.
Therefore, we provide graphical summaries based on the ESS. We can visualise
this metric by plotting histograms of the effective sample size across all 
genes using `BASiCS_DiagHist`, or by plotting effective sample size against
mean expression or over-dispersion for all genes using `BASiCS_DiagPlot`.
As seen in Figure \@ref(fig:diag-plot-naive), most genes have effective sample 
sizes near 1,000. Furthermore, Figure \@ref(fig:geweke-diag-ess) demonstrates 
...
```{r diag-hist-naive, fig.cap="Distribution of effective sample size across all genes for mean and over-dispersion parameters in naive cells. A dashed line indicates the minimum effective sample size for genes to be considered in differential expression tests.", eval=FALSE,echo=FALSE}
...
```{r geweke-diag-ess, fig.cap="Geweke diagnostic criterion for mean expression plotted against effective sample size in naive cells."}
ggplot() +
  aes(
    ess_naive,
    gew_mu_naive
  ...
  geom_hex() +
  labs(x = "Effective sample size", y = "Geweke diagnostic for mean expression") +
  geom_hline(yintercept = c(-3, 3), col = "firebrick", linetype = "dashed") +
  scale_fill_viridis()
```
...
Several genes show very low effective sample size for mu. These genes may 
indicate systematic problems with the MCMC algorithm; thus,
it may be useful to identify these genes.
`BASiCS_EffectiveSize` allows the calculation of effective sample size for
any parameters in a `BASiCS_Chain` object.

```{r low-ess-genes}
ess <- BASiCS_EffectiveSize(chain_naive, Param = "mu")
...
reliably estimate changes in mean expression. Therefore it is appropriate 
...
plot(chain_active, Param = "mu", Gene = 1)
```


```{r ess-diag-hist, fig.cap="Distribution of effective sample size across all genes for mean and over-dispersion parameters in active cells. A dashed line indicates the minimum effective sample size for genes to be considered in differential expression tests.", eval=FALSE, echo=FALSE}
mu_ess <- BASiCS_DiagHist(chain_active, Param = "mu") +
  xlab("ESS: mu") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
  geom_vline(xintercept = 100, linetype = "dashed", colour = "grey60")
delta_ess <- BASiCS_DiagHist(chain_active, Param = "delta") +
  xlab("ESS: mu") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
  geom_vline(xintercept = 100, linetype = "dashed", colour = "grey60")
multiplot(mu_ess, delta_ess, cols = 2)
...
```{r ess-diag-plot, fig.cap="Effective sample size is plotted against parameter estimates for mean and over-dispersion parameters in active cells. A dashed line indicates the minimum effective sample size for genes to be considered in differential expression tests. Colour indicates local density, with lighter colours representing higher density."}
mu_ess2 <- BASiCS_DiagPlot(chain_active, Param = "mu") +
  theme(
    legend.position = "bottom",
  axis.text.x = element_text(hjust = 1, angle = 45)) +
  geom_hline(yintercept = 100, linetype = "dashed", colour = "grey60")
delta_ess2 <- BASiCS_DiagPlot(chain_active, Param = "delta") +
  theme(
    legend.position = "bottom",
    axis.text.x = element_text(hjust = 1, angle = 45)) +
  geom_hline(yintercept = 100, linetype = "dashed", colour = "grey60")
...
### Data normalisation using BASiCS

To highlight the use of `r Biocpkg("BASiCS")` to analyse cells in the 
single-condition case, we select naive CD4^+^ T cells of young B6 animals.
Here, `r Biocpkg("BASiCS")` can be used to identify highly- and lowly-variable 
genes and calculate interpretable gene- and cell-specific parameters.
...
In this section, we will highlight the use of `r Biocpkg("BASiCS")` when
analysing cells of a single condition. This includes normalisation, variance 
decomposition, detection of highly and lowly variable genes, and the use of
gene-specific parameters as interpretable variability measures.
...
To perform normalisation, `r Biocpkg("BASiCS")` provides the 
`BASiCS_DenoisedCounts` and the `BASiCS_DenoisedRates` functions. These 
functions produce normalised expression values, with technical variation 
removed. They differ in that the former returns a denoise expression count, 
such that
$$
...
counts_denoised <- BASiCS_DenoisedCounts(Data = sce_naive, Chain = chain_naive)
```
...
basics_size_factors <- colMedians(
  displayChainBASiCS(chain_naive, "phi")
  * displayChainBASiCS(chain_naive, "nu")
)
scran_size_factors <- sizeFactors(sce_naive)
g1 <- ggplot(data.frame(basics_size_factors, scran_size_factors)) +
  aes(basics_size_factors, scran_size_factors) +
  geom_point() +
  ggtitle("Naive")
...
scran_size_factors <- sizeFactors(sce_active)
g2 <- ggplot(data.frame(basics_size_factors, scran_size_factors)) +
  aes(basics_size_factors, scran_size_factors) +
  geom_point() +
  ggtitle("Active")
...
An advantage of jointly inferring normalisation rather than using pre-computed
normalisation factors is that uncertainty is propagated from the normalisation
step to all downstream analyses.


## Quantification of variability using BASiCS
...
with mean expression. The same is true for the over-dispersion parameters 
estimated by `r Biocpkg("BASiCS")`, as shown in Figure
\@ref(fig:CV-comparison-BASiCS). Again, for this comparison,
...
Furthermore, Figure \@ref(fig:epsilon-dm) shows that these parameters are highly
correlated.
...
`r Biocpkg("BASiCS")` offers a function to select genes with large or small 
...
## Differential expression testing using BASiCS
...
in Figure \@ref(fig:violin-plots).
...
Here, we filter on genes that are lowly expressed in both conditions.
...
a large effective sample size, similar to the case when we included spike-in
molecules.
