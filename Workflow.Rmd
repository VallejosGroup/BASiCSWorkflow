...
select_all <- c(select_top_n(table_de, "PSM+"), # up-regulated in pre-somitic cells
                ...
                select_top_n(table_de, "NoDiff")) # not differentially expressed
counts_psm <- assay(sce_psm, "BASiCS_norm")[select_all,]
counts_sm <- assay(sce_sm, "BASiCS_norm")[select_all,]
rownames(counts_psm) <- rownames(counts_sm) <- rowData(sce_sm)[select_all, "external_gene_name"]

## split heatmaps by gene category
split <- data.frame(
  Upregulated = c(
    rep("Up-regulated \nin PSM", length(select_all)/3),
    rep("Up-regulated \nin SM", length(select_all)/3),
    rep("Non DE", length(select_all)/3)
  )
)
fontsize <- 7
