...
to obtain.
Here, we remove genes that are not detected in at least 5 cells across all 
...
plotRowData(
  ...
  ylab("Average number of read counts across cells") +
  scale_x_log10() +
  scale_y_log10() +
  theme(
    legend.position = "bottom",
...
which compares the average of draws obtained during the initial (10\% after burn 
in, by default) and the average of draws obtained during the final part of the 
chain (50\% by default).
...
(see Figure \@ref(fig:geweke-diag)), suggesting that the algorithm has 
largely converged.
...
gew_mu_naive <- gew_mu_naive_obj$z
gew_mu_active_obj <- geweke.diag(
  mcmc(displayChainBASiCS(chain_active, Param = "mu"))
)
gew_mu_active <- gew_mu_active_obj$z
