...
Practical guidance about MCMC convergence diagnostics is provided in the next
section.
...
This first of these samplers takes 84 minutes to complete on a 3.4 GHz Intel
...
As expected, changes in residual over-dispersion are notconfounded by changes 
in mean expression Figure \@ref(fig:combine-results). 
...
ggplot(table_de_combined) +
  ...
  plot_params +
  xlim(-15, 15) +
  labs(x = "log2 fold change in mean expression", 
...
While genes with differences in residual over-dispersion have similar
...
table_de_combined <- merge(table_de_combined, gene_tests)
...
g1 <- ggplot(
    table_de_combined[table_de_combined$ResultDiffResDisp == "Naive+", ]
  ) +
  ...
  geom_abline(
    slope = 0, intercept = 0, colour = "firebrick", linetype = "dashed"
  )

g2 <- ggplot(
    table_de_combined[table_de_combined$ResultDiffResDisp == "Active+", ]
  ) +
  aes(MeanOverall, MeanLog2FC) +
  plot_params +
  ylim(-15, 15) +
...
showing a different type of differential variability. Figure
...
higher levels of residual over-dispersion in naive or active cells (A and C, 
respectively). These are genes with higher variability in one population that
is not driven by a higher proportion of zeros and may indicate more stochastic 
expression patterns.
...
higher levels of residual over-dispersion in naive or active cells (B and D, 
respectively). These are genes with higher variability in one population that
...
plot_vg <- function(ind_vg) {
  table_de_combined_vg <- table_de_combined[ind_vg, ]
  # If more than 4 genes, pick top 4 ranked by differences in residual over-dispersion
  if(nrow(table_de_combined_vg) > 4) {
    table_de_combined_vg <- table_de_combined_vg[
      order(abs(table_de_combined_vg$ResDispDistance), decreasing = TRUE),]  
    table_de_combined_vg <- table_de_combined_vg[1:4,]
  }
  var_genes <- table_de_combined_vg$GeneName
...
ind_vg <- table_de_combined$ResultDiffResDisp %in% c("Naive+") &
  table_de_combined$ResultDiffMean %in% c("NoDiff") &
  abs(table_de_combined$ExpPropNaive - table_de_combined$ExpPropActive) < 0.05 &
  pmin(table_de_combined$ExpPropNaive, table_de_combined$ExpPropActive) > 0.75
g1 <- plot_vg(ind_vg) +
    theme(legend.position = "none")
...
ind_vg <- table_de_combined$ResultDiffResDisp %in% c("Naive+") &
  table_de_combined$ResultDiffMean %in% c("NoDiff") &
  abs(table_de_combined$ExpPropNaive - table_de_combined$ExpPropActive) > 0.40 &
  pmax(table_de_combined$ExpPropNaive, table_de_combined$ExpPropActive) > 0.75
g2 <- plot_vg(ind_vg)
...
ind_vg <- table_de_combined$ResultDiffResDisp %in% c("Active+") &
  table_de_combined$ResultDiffMean %in% c("NoDiff") &
  abs(table_de_combined$ExpPropNaive - table_de_combined$ExpPropActive) < 0.05 &
  pmin(table_de_combined$ExpPropNaive, table_de_combined$ExpPropActive) > 0.75
g3 <- plot_vg(ind_vg) +
...
ind_vg <- table_de_combined$ResultDiffResDisp %in% c("Active+") &
  table_de_combined$ResultDiffMean %in% c("NoDiff") &
  abs(table_de_combined$ExpPropNaive - table_de_combined$ExpPropActive) > 0.20 &
  pmax(table_de_combined$ExpPropNaive, table_de_combined$ExpPropActive) > 0.50
g4 <- plot_vg(ind_vg)
...
- **R packages**: 
  ...
  Version numbers for all remaining packages are available at
