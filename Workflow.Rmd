...
rowData(sce_naive_active)$include_gene <- rowData(sce_naive_active)$mean >= 1 &
  rowData(sce_naive_active)$detected_cells >= 10
plotRowData(
    sce_naive_active,
    x = "detected_cells",
    y = "mean",
...
This first of these samplers takes 227 minutes to complete on a 3.4 GHz Intel 
Core i7 4770k procesor with 16GB RAM, while the second takes 154 minutes.
...
chain_naive <- readRDS("rds/chain_naive_cata.Rds")
chain_active <- readRDS("rds/chain_active_cata.Rds")
```
...
```{r mcmc-diag, fig.cap="MCMC diagnostics for gene-specific mean expression parameters; naive CD4^+^T cells. A: Geweke Z-score for mean expression parameters is plotted against mean expression estimates. B: Effective sample size (ESS) is plotted agains mean expression estimates."}
library("coda")
library("ggplot2")
library("viridis")

# Calculate and plot Geweke Z scores 
gew_mu_naive_obj <- geweke.diag(
  mcmc(displayChainBASiCS(chain_naive, Param = "mu"))
)
geweke_mu_naive <- ggplot() +
  aes(
    colMedians(displayChainBASiCS(chain_naive, Param = "mu")),
    gew_mu_naive_obj$z
  ) +
  geom_hex() +
  scale_x_log10() + 
...
ggplot(VG_Table) +
  ...
  scale_colour_manual(name = NULL,
    values = c(HVG = "firebrick", LVG = "dodgerblue"),
    na.value = "grey80"
  )
```
...
plot_hvg <- ggplot(melt(topHVG), aes(x = Var2, y = value)) + 
  plot_params + ggtitle("Example HVG")

plot_lvg <- ggplot(melt(topLVG), aes(x = Var2, y = value)) + 
  plot_params + ggtitle("Example LVG")
...
TableMean <- as.data.frame(Test_DE, Parameter = "Mean", Filter = FALSE)
TableResDisp <- as.data.frame(Test_DE, Parameter = "ResDisp", Filter = FALSE)
```
...
```{r combine-results}
TableCombined <- merge(TableMean, TableResDisp)
```
...
Figure \@ref(fig:violin-diffresdisp) shows denoised counts for five genes with higher residual overdispersion in naive cells. Here, we show only genes with 
similar mean expression.


```{r, eval = FALSE, echo = FALSE}
# Tests
myDF <- data.frame("GeneName" = rownames(DC_naive),
...
myDF <- myDF[!grepl("ERCC", myDF$GeneName),]
dim(myDF)
dim(TableCombined)
myDF <- merge(TableCombined, myDF)
...
g1 <- ggplot(myDF[myDF$ResultDiffResDisp == "Naive+",], 
             aes(log10(MeanOverall), MeanLog2FC)) + plot_params + ylim(-18,18) +
  geom_abline(slope = 0, intercept = 0, colour = "firebrick", linetype = "dashed")

g2 <- ggplot(myDF[myDF$ResultDiffResDisp == "Active+",], 
             aes(log10(MeanOverall), MeanLog2FC)) + plot_params + ylim(-18,18) +
  geom_abline(slope = 0, intercept = 0, colour = "firebrick", linetype = "dashed")

g3 <- ggplot(myDF[myDF$ResultDiffResDisp == "Naive+",],
             aes(x = ExpPropNaive, y = ExpPropActive)) + plot_params +
  geom_abline(slope = 1, intercept = 0, colour = "firebrick", linetype = "dashed")

g4 <- ggplot(myDF[myDF$ResultDiffResDisp == "Active+",],
             aes(x = ExpPropNaive, y = ExpPropActive)) + plot_params +
  geom_abline(slope = 1, intercept = 0, colour = "firebrick", linetype = "dashed")
...
myDF <- myDF[order(abs(myDF$ResDispDistance), decreasing = TRUE),]

ind_vg <- myDF$ResultDiffResDisp %in% c("Naive+") & 
  myDF$ResultDiffMean %in% c("NoDiff") &
  abs(myDF$ExpPropNaive - myDF$ExpPropActive) < 0.10 &
  (myDF$ExpPropNaive + myDF$ExpPropActive)/2 > 0.5
var_genes <- myDF$GeneName[ind_vg]
length(var_genes)
...
ind_vg <- myDF$ResultDiffResDisp %in% c("Active+") & 
  myDF$ResultDiffMean %in% c("NoDiff") &
  abs(myDF$ExpPropNaive - myDF$ExpPropActive) < 0.10 &
  (myDF$ExpPropNaive + myDF$ExpPropActive)/2 > 0.5
var_genes <- myDF$GeneName[ind_vg]
...
```{r violin-diffresdisp, fig.width = 7, fig.height = 5, fig.cap="Violin plot of denoised counts for five genes with higher residual overdispersion in naive cells."}
library("tidyr")
#TableCombined <- TableCombined[order(TableCombined$ProbDiffResDisp, decreasing = TRUE),]
#ind_vg <- #abs(TableCombined$MeanLog2FC) < 0.5 &
#  log10(TableCombined$MeanOverall) > 2 &
#  log10(TableCombined$MeanOverall) < 2.5  &
#  abs(TableCombined$ResDispDistance) > 2 &
#  TableCombined$ResultDiffMean %in% c("NoDiff") &
#  TableCombined$ResultDiffResDisp %in% c("Naive+")
#table(ind_vg)
#var_genes <- TableCombined$GeneName[ind_vg]
...
ind_vg <- #abs(TableCombined$MeanLog2FC) < 0.1 &
  #log10(TableCombined$MeanOverall) > 2 &
  #log10(TableCombined$MeanOverall) < 2.5 &
  TableCombined$ResultDiffMean %in% c("NoDiff") &
  TableCombined$ResultDiffResDisp %in% c("Active+")
table(ind_vg)
var_genes <- TableCombined$GeneName[ind_vg]
var_genes <- var_genes[1:3]
var_naive <- DC_naive[var_genes, ]
