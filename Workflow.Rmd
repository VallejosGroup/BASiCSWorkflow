...
We can observe that changes in residual over-dispersion are not 
confounded by changes in mean expression Figure \@ref(fig:combine-results). 
...
and vice versa. Thus, these genes may represent those with a more intermittent
...
respectively). These are genes with higher variability in one population
than the other. Given that these genes have a similar proportion of zero counts
in both populations, it is likely that this difference in variability
is not driven by an more intermittent gene expression pattern.
...
may be driven by more intermittent expression patterns. These may point to 
...
ind_vg <- table_combined_genes$ResultDiffResDisp %in% c("Naive+") &
  ...
  abs(table_combined_genes$ExpPropNaive - table_combined_genes$ExpPropActive) < 0.05 &
  pmin(table_combined_genes$ExpPropNaive, table_combined_genes$ExpPropActive) > 0.75
g1 <- plot_vg(ind_vg) +
    theme(legend.position = "none")
...
ind_vg <- table_combined_genes$ResultDiffResDisp %in% c("Naive+") &
  table_combined_genes$ResultDiffMean %in% c("NoDiff") &
  abs(table_combined_genes$ExpPropNaive - table_combined_genes$ExpPropActive) > 0.40 &
  pmax(table_combined_genes$ExpPropNaive, table_combined_genes$ExpPropActive) > 0.75
g2 <- plot_vg(ind_vg)
...
ind_vg <- table_combined_genes$ResultDiffResDisp %in% c("Active+") &
  table_combined_genes$ResultDiffMean %in% c("NoDiff") &
  abs(table_combined_genes$ExpPropNaive - table_combined_genes$ExpPropActive) > 0.20 &
  pmax(table_combined_genes$ExpPropNaive, table_combined_genes$ExpPropActive) > 0.50
g4 <- plot_vg(ind_vg)
