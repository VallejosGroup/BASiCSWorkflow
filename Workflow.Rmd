...
ecosystem to analyse scRNA-seq datasets [@Lun2016; @Kim2019]. 
We present a step-by-step computational workflow which combines 
`r Biocpkg("scater")` and `r Biocpkg("scran")` to perform quality control steps 
as well as low-level preliminary analysis [@McCarthy2017; @Lun2016]. 
...
that simultaneously performs data normalisation (global scaling), technical 
noise quantification and selected downstream analyses, whilst propagating 
...
biological components [@Vallejos2015]. 
...
HVG detection is often used as a feature selection step to identify the input 
set of genes for subsequent analyses. 
The variance decomposition obtained by `r Biocpkg("BASiCS")` can also be used
to highlight *lowly variable genes* (LVGs) that exhibit stable expression across
the a population of cells.
LVGs may relate to essential cellular functions and can assist the development
of new data normalisation or integration strategies [@Lin2019]. 

<!--- BASiCS downstream analyses between populations --->
Often, scRNA-seq analyses aim to compare expression patterns across two or more
pre-specified groups of cells, such as experimental conditions or cell types.
In this context, `r Biocpkg("BASiCS")` provides a probabilistic decision rule 
for differential expression analyses [@Vallejos2016; @Eling2018]. 
Whilst several differential expression tools have been proposed for scRNA-seq 
data (e.g. [@Kharchenko2014; @Finak2015]), some evidence suggests that these do 
not generally outperform popular bulk RNA-seq tools [@Soneson2018]. 
Moreover, most of these methods are unable to uncover the more complex 
differential expression patterns (beyond changes in overall expression) that 
can arise at the single cell level [@Lahnemann2020]. 
...
This document is structured as follows. Firstly, we briefly discuss the 
different sources of variability that can be reflected in scRNA-seq datasets 
and some of the strategies that have been designed to control or attenuate 
technical noise in scRNA-seq experimental protocols. 
We also summarise the main features of the Bioconductor packages that are used
throughout this workflow, including a description for the underlying statistical 
...
Here, we briefly describe the underlying sources of variability that can be 
captured by estimates derived from scRNA-seq data. 

Stochastic variability within a population of cells is often referred to as
transcriptional *noise* and can arise from intrinsic and extrinsic sources
...
noise that is typically observed in scRNA-seq assays [@Brennecke2013].
...
[@Vallejos2017] and be used to infer background technical noise 
[@Brennecke2013].
