...
profiles between the identified clusters [@Lahnemann2020]). 
...
calculate QC metrics for each cell (e.g. total read-count) and gene (e.g. 
percentage of zeroes across all cells), respectively. 
...
[@Lun2016pooling]. Note that, as described in the next section, the use of 
`r Biocpkg("scran")` as a normalisation tool is not a requirement for 
`r Biocpkg("BASiCS")` (which uses raw read-counts as input). However, we
recommend the use of `r Biocpkg("scran")` normalisation as part of the 
exploratory data analysis. For example, as illustrated in this workflow, 
normalised read-counts can be used to visualise the data in a low-dimensional
space (e.g. using principal component analysis). This can help to explore the
underlying structure of the data, e.g. due to batch effects or other systematic
differences (e.g. associated to different experimental conditions) across 
different groups of cells. 

## BASiCS - Bayesian Analysis of Single Cell Sequencing data

Instead of modelling expression patterns for each gene separately, the 
`r Biocpkg("BASiCS")` Bioconductor package uses a Bayesian hierarchical 
framework that shares information between all genes and cells to robustly
quantify transcriptional variability [@Vallejos2015BASiCS]. 
For example, as described by [@Eling2018], pooling information across genes with 
similar mean expression levels helps to obtain more reliable transcriptional 
variability estimates. The latter is particularly helpful for lowly expressed 
genes and sparse datasets, where less information is available.

As shown in Figure [ADD REF TO DIAGRAM], the model implemented in 
`r Biocpkg("BASiCS")` involves two broad classes of parameters designed to 
capture different aspects of the variability observed in scRNAseq data. The 
first class includes *nuisance* parameters whose role is to (primarily) capture 
technical artifacts. This includes cell-specific parameters (indexed by a $j$
subscript) used to perform global scaling normalisation (similar to the size 
factors in `r Biocpkg("scran")`) and global parameters designed to capture 
technical over-dispersion (this can also be interpreted as measurement error) 
that is shared by all genes. Whilst `r Biocpkg("scran")` infers cell-specific 
size factors as a pre-processing step, `r Biocpkg("BASiCS")` uses an integrated 
approach wherein data normalisation and downstream analyses are performed 
simultaneously, thereby propagating statistical uncertainty. 

The second class comprises gene-specific (indexed by an $i$ subscript) whose 
role is to characterise the underlying gene expression distribution within the 
cell population under study. This could correspond to a group of cells that is 
set *a priori* by the experimental design (e.g. naive or stimulated 
...
differential expression tools). `r Biocpkg("BASiCS")` summarises the expression 
pattern for each gene through gene-specific *mean* and *over-dispersion* 
parameters. Mean parameters $\mu_i$ quantify the overall expression for each 
gene $i$ across the cell population under study. In contrast, $\delta_i$ 
...
within the analysed cells [@Brennecke2013]. HVG detection is often used as 
feature selection, to choose the input set of genes for subsequent analyses. A 
similar rule was developed to highlight *lowly variable genes* (LVGs) that 
exhibit stable expression across the population of cells. These may relate to 
essential cellular functions and can assist the development of new data 
normalisation or integration strategies [@Lin2019].

- **@Vallejos et al (2016)** [@Vallejos2016]: the model was extended to enable
**differential expression** analysis between two pre-specified groups of cells 
...
cell-to-cell transcriptional variability. [SAY SOMETHING ABOUT THE CONFOUNDING]. 
This limitation was addressed by @Eling2018. 

- **@Eling et al (2018)** [@Eling2018]: 
...
`r Biocpkg("BASiCS")` uses information from , but the model has been extended to address situations wherein
...
spike-ins is provided in the supplementary material, Section [ADD-REF]). 
...
the supplementary material (Section [ADD-REF]). 
...
exploratory data analysis in the supplementary material (see Section [ADD-REF]). 
...
See Section [ADD-REF] in the supplementary material for further examples on
how to explore the posterior distribution. 
...
all MCMC chains. The convergence analysis for the chains generated for pre-somatic 
mesoderm cells is provided in Section [ADD-REF] within the supplementary material. 
...
obtained for pre-somitic mesoderm cells in the supplementary material). 
...
## We use `plot_params` to specify the format of such plots whilst avoiding code duplication
...
g2 <- ggplot(parameter_df[sel_genes,], aes(log10(mean_scran), log10(cv2_scran))) +
  xlim(-3, 2.2) + ylim(-3, 3) 
...
have observed in the context of less sparse data (see Section [ADD-REF] in the 
supplementary material). 
For the majority of genes (`r with(parameter_df[sel_genes,], sum(sign(DM) == sign(residualoverdisp)))` out of `r sum(sel_genes)` captured in at least 3 
cells) both approaches led some broadly similar results, inferring the same 
sign for their residual variability estimates. However, [TO-DO: explore what
drives the difference; rank figure below is a tentative idea].
...
```{r rank-var-comparison}
g1 <- ggplot(parameter_df[sel_genes,], aes(rank(DM), rank(residualoverdisp))) 
# Select genes captured in at least 100 cells 
sel_genes2 <- sel_genes & rowSums(counts(sce_sm) > 0) > 100
g2 <- ggplot(parameter_df[sel_genes2,], aes(rank(DM), rank(residualoverdisp))) 

(g1 + g2) * plot_params + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 15))
```
...
## The plotExpression() function from the scater package is used for this purpose
...
```{r offset-denoised}
## Calculate a global offset between the groups using the SM population as reference
...
## utility function for selecting top DE/non-DE genes
...
select_all <- c(
  select_top_n(table_de, "PSM+"), # up-regulated in pre-somitic cells
  select_top_n(table_de, "SM+"), # up-regulated in somitic cells
  select_top_n(table_de, "NoDiff", decreasing = FALSE) # not differentially expressed
) 
counts_psm <- assay(sce_psm, "BASiCS_norm")[select_all,]
counts_sm <- assay(sce_sm, "BASiCS_norm")[select_all,]
rownames(counts_psm) <- rownames(counts_sm) <- 
  rowData(sce_sm)[select_all, "external_gene_name"]
...
plot_vg <- function(ind_vg) {
  ...
  colnames(var_df)[-ncol(var_df)] <- rowData(sce_sm)[var_genes, "external_gene_name"]
...
ind_vg <- table_de_genetests$ResultDiffResDisp %in% c("SM+") &
...
ind_vg <- table_de_genetests$ResultDiffResDisp %in% c("PSM+") &
  table_de_genetests$ResultDiffMean %in% c("NoDiff") &
  abs(
    table_de_genetests$ExpPropSM - table_de_genetests$ExpPropPSM
  ) < 0.1 &
...
```{r, echo = FALSE}
# saveRDS(sce_sm, "rds/sce_sm_output.rds")
# saveRDS(sce_psm, "rds/sce_psm_output.rds")
```
