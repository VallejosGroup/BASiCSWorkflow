...
we use `r CRANpkg("ggpointdensity")` to visualise 
...
```{r, violin-diffresdisp, fig.width = 7, fig.height = 5, fig.cap="Violin plots of denoised counts. A: three genes with higher residual over-dispersion in naive cells, and similar levels of detection in active and naive populations. B: three genes with higher residual over-dispersion in naive cells and different levels of detection in naive and active cells. C: three genes with higher residual over-dispersion in active cells and similar levels of detection in naive and active cells. D: three genes with higher residual over-dispersion in active cells and different levels of detection in naive and active cells."}
...
based on the knowledge that the original quantity of spike-in molecules was
...
relying on spike-ins to quantify technical noise.
...
Using `r Biocpkg("BASiCS")` without spike-ins is a very similar process
to the process of using it with spike-ins. We will demonstrate using the
...
```{r mu-spike-comp, fig.cap="Comparison of mean expression point estimates using spike-ins, and the same parameters estimated without using spike-ins."}
...
```{r delta-spike-comp, fig.cap="Comparison of over-dispersion point estimates using spike-ins, and the same parameters estimated without using spike-ins."}
