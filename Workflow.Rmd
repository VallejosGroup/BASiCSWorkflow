...
`r CRANpkg("ggplot2")`) and colour scales (e.g., `r CRANpkg("viridis")`. 
...
quantification and downstream analyses [@Vallejos2015BASiCS;@Vallejos2016;@Eling2018]. Moreover, instead of modelling expression patterns separately for 
...
shown in Figure X (a more detailed description is provided in [TODO: REPO LINK]).
...
Additionally, we use a *joint prior* specification for 
these parameters. This joint prior assumes that genes with similar mean 
expression ($\mu_i$) have similar over-dispersion ($\delta_i$) parameters.
...
groups of cells [@Vallejos2016;@Eling2018] in terms of either mean
expression or residual over-dispersion.
...
the model that we will focus on here, and that we recommend for most
...
<!--- However, [TO-DO: explore what
drives the difference; rank figure below is a tentative idea].

Do we need this? If this is a workflow for analysing data using BASiCS,
we should focus on what BASiCS does.
...
# (g1 + g2) *
#   plot_params + plot_annotation(tag_levels = "A") &
...
select_all <- c(
  ...
  select_top_n(table_de, "NoDiff", decreasing = FALSE) # no difference
) 
...
plot_vg <- function(ind_vg) {
  ...
  var_long_df <- pivot_longer(var_df, cols = var_gene_names)
