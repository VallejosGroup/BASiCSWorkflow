...
for which the data contains limited information. 
Here, we remove all genes that are not detected in at least 5 cells across both 
experimental conditions or for which their average scran normalised read count 
...
```{r gene-selection, fig.cap="Average expression for each gene is plotted against the number of cells in which that gene was detected. Dashed grey lines are shown at the thresholds below which genes are removed."}
...
rowData(sce_naive_active)$detected_cells <- rowData(sce_naive_active)$detected * ncol(sce_naive_active) / 100
...
# Apply gene filter 
sce_naive_active <- sce_naive_active[include_gene, ]
```
...
spikes_naive <- assay(altExp(sce_naive, "spike-ins"))
spikes_active <- assay(altExp(sce_active, "spike-ins"))
spikes_present <- rowSums(spikes_naive) != 0 & rowSums(spikes_active) != 0
## Remove spike-ins that are not present from matrix and SCE object
altExp(sce_naive, "spike-ins") <- altExp(sce_naive)[spikes_present, ]
altExp(sce_active, "spike-ins") <- altExp(sce_active)[spikes_present, ]
```
