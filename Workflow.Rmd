...
contain broken cells, that are empty, or that contain multiple cells 
...
We then convert the data and metadata described above into a 
...
impractical to assess graphical diagnostics for each 
...
ggplot() +
  ...
  geom_hex() +
  labs(x = "log(mu)", y = "Geweke diagnostic\nfor mean expression") +
...
ggplot() +
  ...
  geom_hex() +
  labs(
    x = "Proportion of cells detected",
    y = "Geweke diagnostic\nfor mean expression") +
  geom_hline(yintercept = c(-3, 3), col = "firebrick", linetype = "dashed") +
  scale_fill_viridis()
```  
...
We provide graphical summaries based on the ESS in order to explore the 
efficiency of the MCMC algorithm across a large number of parameters
simultaneuously. We can visualise
...
`BASiCS_TestDE` automatically excludes genes with low ESS from tests of 
