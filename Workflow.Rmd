...
g2 <- ggplot() +
  ...
  annotate(
    ...
    label = paste(
      "Pearson's r = ", 
      round(
        cor(basics_sf_active, scran_sf_active), 
        digits = 2
      )
    ), 
    geom = "text",
...
[@Shalek2014] and the squared coefficient of variation (CV^2^)
(CV, variance divided by squared mean expression) 
[@Buettner2015;@Brennecke2013].
In contrast, `r Biocpkg("BASiCS")` uses gene-specific over-dispersion 
parameters $\delta_i$ as a proxy for transcriptional variability within a
population of cells. 
...
and how these relate to mean expression estimates. To generate this plot,
we can use the `BASiCS_ShowFit` function. This plots the gene-specific 
over-dispersion parameters (delta) versus mean expression parameters (mu).

```{r dispersion-vs-mean, fig.cap="Over-dispersion is plotted against mean expression for naive and active cells on a log-log scale. Colour corresponds to local density, with lighter colours representing higher density; a curve represents the inferred trend of over-dispersion against mean, while the shaded area represents the corresponding 95\\% high density interval."}
fit_plot_naive <- BASiCS_ShowFit(chain_naive) +
  theme_bw() +
  ggtitle("Naive cells")
fit_plot_active <- BASiCS_ShowFit(chain_active) +
  theme_bw() +
  ggtitle("Active cells")
multiplot(fit_plot_naive, fit_plot_active, cols = 2)
```

Recently [@Eling2018], we extended `r Biocpkg("BASiCS")` to explicitly model 
the mean-variability relationship by performing an internal regression between 
the over-dispersion and mean expression parameters, as visualised in Figure 
\@ref(fig:dispersion-vs-mean). This results in the $\epsilon$ parameter, a measure of variability that is not confounded by mean expression. This *residual* 
over-dispersion parameter quantifies the level of variability above or below 
that expected based on the overall mean-variance relationship.
Similarly, Kolodziejczyk _et al._ used the distance to a rolling 
median (DM) along the mean-variability trend to correct for this confounding 
factor [@Kolodziejczyk2015cell]. Here, we highlight how to obtain the residual
variability estimates using `r Biocpkg("BASiCS")` and `r Biocpkg("scran")`.
Further, we illustrate the mean-variance relationship for each variability 
measures. For this analysis, we exclude genes that are not expressed in at 
least 2 cells. These genes have missing (`NA`) values for the residual 
over-dispersion parameter, $\epsilon$. These values can be obtained using the
function `displayChainBASiCS`. We will use `r CRANpkg("ggpointdensity")` 
to visualise the local density of genes along the axes of mean and variability.
...
## normalise counts using scran size factors
...
parameter_df <- data.frame(
  ...
  ## mean and variance of scran-normalised counts
  mean_scran_naive = rowMeans(assay(sce_naive, "normcounts")),
  mean_scran_active = rowMeans(assay(sce_active, "normcounts")),
  var_scran_naive = rowVars(assay(sce_naive, "normcounts")),
  var_scran_active = rowVars(assay(sce_active, "normcounts"))
)
## Use scran mean and variance to calculate cv2 and DM
parameter_df$cv2_scran_naive <- parameter_df$var_scran_naive /
  parameter_df$mean_scran_naive^2
parameter_df$cv2_scran_active <- parameter_df$var_scran_active /
  parameter_df$mean_scran_active^2

parameter_df$DM_naive <- DM(
  mean = parameter_df$mean_scran_naive,
  cv2 = parameter_df$cv2_scran_naive
)
parameter_df$DM_active <- DM(
  mean = parameter_df$mean_scran_active,
  cv2 = parameter_df$cv2_scran_active
)

## Remove genes without counts in > 2 cells
ind_not_na <- !(is.na(parameter_df$eps_naive) | is.na(parameter_df$eps_active))

p_eps_naive <- ggplot(parameter_df[ind_not_na, ]) +
  ...
  labs(
    x = "Mean expression",
    y = "Residual over-dispersion",
    title = "Naive cells"
  )
p_eps_active <- ggplot(parameter_df[ind_not_na, ]) +
...
Figures \@ref(fig:delta-var) and \@ref(fig:delta-cv2) show that variance and
CV^2^ are both highly correlated with the over-dispersion estimates from 
`r Biocpkg("BASiCS")`. Again, for this comparison,
...
observation. Furthermore, Figure \@ref(fig:dispersion-vs-mean) shows that the 
over-dispersion parameters estimated by `r Biocpkg("BASiCS")` are strongly
correlated with mean expression. 


```{r delta-var, fig.cap="Over-dispersion estimates obtained by BASiCS are plotted against variance of scran-normalised counts (log-scale). Colour represents local density, with lighter colours representing higher density. Genes that are not observed to be expressed in at least 2 cells within each group are excluded."}
p_delta_var_naive <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(log(delta_naive), log(var_scran_naive))) +
  scale_colour_viridis() +
  theme(
    legend.position = "bottom",
    legend.text = element_text(hjust = 1, angle = 45)
...
p_delta_var_active <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(log(delta_active), log(var_scran_active))) +
  scale_colour_viridis() +
  theme(
    legend.position = "bottom",
    legend.text = element_text(hjust = 1, angle = 45)
...
multiplot(
  p_delta_var_naive, p_delta_var_active,
  cols = 2
)
```


```{r delta-cv2, fig.cap="b"}
p_delta_cv2_naive <- ggplot(parameter_df[ind_not_na, ]) +
...
p_delta_cv2_active <- ggplot(parameter_df[ind_not_na, ]) +
...
```{r mean-dm, fig.cap="DM estimates obtained using scran are plotted against mean expression (log-scale). Colour represents local density, with lighter colours representing higher density. Genes that are not observed to be expressed in at least 2 cells within each group are excluded."}
p_dm_naive <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(log(mean_scran_naive), DM_naive)) +
  scale_colour_viridis() +
  theme(legend.position = "bottom") +
  labs(
    x = "Mean expression",
...
p_dm_active <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(log(mean_scran_active), DM_active)) +
  scale_colour_viridis() + 
  theme(legend.position = "bottom") +
  labs(
    x = "Mean expression",
...
```{r eps-dm, fig.cap = "c"}
p_eps_DM_naive <- ggplot(parameter_df[ind_not_na, ]) +
  ...
  labs(
    x = "Residual over-dispersion",
    y = "DM",
    title = "Naive cells"
  )
p_eps_DM_active <- ggplot(parameter_df[ind_not_na, ]) +
