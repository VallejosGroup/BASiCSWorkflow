...
identified through clustering. In such cases, the model implemented in 
`r Biocpkg("BASiCS")` does not currently address issues associated to 
post-selection inference, where the same data is analysed twice: first to 
perform clustering and then to compare expression profiles between clusters 
[@Lahnemann2020]. 
...
For example, as described below, we pool information across genes with similar 
expression levels to obtain more reliable parameter estimates. The latter is 
particularly helpful for lowly expressed genes and sparse datasets, where less 
...
not use spike-ins; however, we include an example of using data generated
using spike-ins in TODO: Supplementary.
...
```{r}
diag_p1 <- BASiCS_DiagPlot(chain_sm, Param = "delta", Measure = "geweke")
diag_p1 <- diag_p1 +
  geom_hline(yintercept = c(-3, 3), col = "firebrick", linetype = "dashed") +
  theme(legend.position = "none")
diag_p2 <- BASiCS_DiagPlot(chain_sm, Param = "delta", Measure = "ess")
diag_p2 <- diag_p2 +
  geom_hline(yintercept = 100, col = "firebrick", linetype = "dashed") +
  theme(legend.position = "none")
diag_p1 + diag_p2 + plot_annotation(tag_levels = "A")
