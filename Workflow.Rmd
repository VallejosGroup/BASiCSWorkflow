...
select_all <- c(select_top_n(table_de, "PSM+"), # up-regulated in pre-somitic cells
...
split <- data.frame(
  Upregulated = rep(c("Up-regulated \nin PSM", 
                      "Up-regulated \nin SM",
                      "Non DE"), each = length(select_all)/3))
fontsize <- 7
...
match_order <- match(rownames(counts_sm), table_de$GeneName)
