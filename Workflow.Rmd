...
development and differentiation), which
can be characterised using pseudotime inference methods [@Saelens2019].
...
of this article. The stochastic component of this variability is referred to as
transcriptional *noise* [@Elowitz2002; @Eling2019]. 
...
variability. Classically, extrinsic noise is defined as a stochastic fluctuation 
in cellular components induced by cells residing in different dynamic 
...
<!--- Briefly describe origins of technical noise --->
<!--- reference other approaches to tackle technical issues (scVI, scran, seurat) --->
...
(such as the ones introduced by the External RNA Controls Consortium, ERCC 
...
<!--- BASiCS ---> 
<!--- what technical issues? should explain/list in previous para --->
...
the visualization of such metrics and expression counts, and the normalisation 
...
of gene-specific squared coefficients of variation (CV2) to a rolling median 
...
function. `BASiCS` also offers a number of functions to visualize and assess the 
...
The normalisation at this point is needed to avoid biases in the visualization 
...
```{r pre-normalisation}
# Pre-normalisation of data for visualization purposes
...
Furthermore, we do detect neither batch effects nor outlier cells based on PCA 
visualisation
...
1 mole is equal to $6.02214076 \times 10^{23}$ molecules.
...
The volume per well in the IFC chip is 9 $nl$
[https://www.fluidigm.com/faq/ifc-9](https://www.fluidigm.com/faq/ifc-9).
We therfore need to calculate the number of molecules in 9 $nl$ reaction mix.
...
change. The absolute amount of spike-ins per reaction is only used to calibrate
the scale of the capture efficiency normalisation parameter $\nu_j$. Since the
true concentration of each spike-in molecule is known, BASiCS uses the observed
counts of each molecule for each cell to estimate the . Differences in the 
...
As an example, we access the first 2 samples for $\mu_i$ of the first 10 genes.
...
There are multiple ways to visualize and 
assess the convergence of MCMC chains
[@CowlesCarlin1996; @BrooksGelman1998].
The `r CRANpkg("coda")`
...
(i) plotting trace plots, sample densities and autocorrelation function, and 
...
The chain is likely to be sampling efficiently if the autocorrelation
...
This includes normalisation, variance decomposition, 
...
clustering as explained elsewhere *TODO: ref,ref*. Alternatively, a 
