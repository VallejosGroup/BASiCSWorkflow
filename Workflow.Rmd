...
output:
  word_document:
    reference_docx: f1000research-11-59-V1.docx
...
context, and are presented in Figure \@ref(fig:fig9-visualise-DE-mean-plot). These
...
```{r fig12-heatmap-diffexp3, fig.width = 5, fig.height = 3.5, fig.cap="Normalised expression values ($\\log_{10}(x + 1)$ scale) for somitic and pre-somitic cells, showing non-differentially expressed genes. For each group, 15 example genes are shown. These were selected according to the ranking of their associated tail posterior probabilities associated to the differential mean expression test. Colour indicates expression level; colour bars on the right of heatmap segments indicate the inferred mean expression level (in log scale) for each gene in each population."}
...
(Figure \@ref(fig:fig13-diff-res-plot)).
...
in mean expression, as shown in Figure \@ref(fig:fig14-combine-results).
...
```{r fig14-combine-results, fig.cap="Difference in residual over-dispersion against $\\log_2$ fold change in mean expression."}
...
Figure \@ref(fig:fig16-violin-diffresdisp) shows denoised counts for
...
Figure \@ref(fig:fig16-violin-diffresdisp)B and \@ref(fig:fig16-violin-diffresdisp)D
...
In contrast, Figure
\@ref(fig:fig16-violin-diffresdisp)A and \@ref(fig:fig16-violin-diffresdisp)C
...
1. URL link to where the software can be downloaded from or used by a non-coder
  https://bioconductor.org/packages/3.18/bioc/html/BASiCS.html
...
A Docker image containing all software requirements is available from
[Docker hub](https://hub.docker.com/r/alanocallaghan/basicsworkflow2020-docker).

This image can be downloaded using the command
`docker pull alanocallaghan/basicsworkflow2020-docker:0.5.3`.
