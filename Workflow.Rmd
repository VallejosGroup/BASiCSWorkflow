...
variability. 
For example, it can infer an overall trend between mean expression 
...
Generally, it is not possible to definitively prove convergence,
but multiple graphical and
...
expression parameters $\mu_i$ were small in absolute value (see Figure \@ref(fig:geweke-diag)), suggesting that the algorithm has largely converged
...
population of cells. 
Figure \@ref(fig:dispersion-vs-mean)A displays posterior estimates for 
over-dispersion parameters inferred for the naive and activated CD4^+^ T cells 
and how these relate to mean expression estimates. To generate such a plot,
...
parameter_df <- data.frame(
  ...
  mean_scran = rowMeans(assay(sce_naive, "normcounts")),
  var_scran = rowVars(assay(sce_naive, "normcounts"))
)

## add scran mean and variance to calculate cv2 and DM
parameter_df$cv2_scran <- parameter_df$var_scran /
...
## delta against mu
g1 <- ggplot(parameter_df[ind_not_na, ]) +
...
## variance against delta
g3 <- ggplot(parameter_df[ind_not_na, ]) +
    geom_pointdensity(aes(log(delta), log(var_scran))) +
    scale_colour_viridis() +
    theme(
      legend.position = "bottom",
      legend.text = element_text(angle = 45, hjust = 1)
...
## cv2 against delta
g4 <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(log(delta), log(cv2_scran))) +
  scale_colour_viridis() +
  theme(
    legend.position = "bottom",
    legend.text = element_text(angle = 45, hjust = 1)
...
## dm against mean
g5 <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(log(mean_scran), DM)) +
  scale_colour_viridis() +
  theme(
    legend.position = "bottom",
    legend.text = element_text(angle = 45, hjust = 1)
...
## dm against epsilon
g6 <- ggplot(parameter_df[ind_not_na, ]) +
...
Here, we focus on naive cells, but a similar analysis for active cells would
show very similar results.


Figures \@ref(fig:dispersion-vs-mean)C and \@ref(fig:dispersion-vs-mean)D 
show that variance and CV^2^ are both highly correlated with the 
over-dispersion estimates from 
...
observation. Furthermore, Figure \@ref(fig:dispersion-vs-mean)A shows that the 
...
with mean expression, as shown in Figures \@ref(fig:dispersion-vs-mean)B
and \@ref(fig:dispersion-vs-mean)E.
Furthermore, these mean-independent variability measures are highly correlated,
as seen in Figure \@ref(fig:dispersion-vs-mean)F.
