...
plot_vg <- function(ind_vg) {
  table_combined_de_vg <- table_combined_de[ind_vg, ]
  # If more than 4 genes, pick top 4 ranked by differences in residual over-dispersion
  if(nrow(table_combined_de_vg) > 4) {
    table_combined_de_vg <- table_combined_de_vg[
      order(abs(table_combined_de_vg$ResDispDistance), decreasing = TRUE),]  
    table_combined_de_vg <- table_combined_de_vg[1:4,]
  }
  var_genes <- table_combined_de_vg$GeneName
...
ind_vg <- table_combined_de$ResultDiffResDisp %in% c("Naive+") &
  ...
  abs(table_combined_de$ExpPropNaive - table_combined_de$ExpPropActive) < 0.05 &
  pmin(table_combined_de$ExpPropNaive, table_combined_de$ExpPropActive) > 0.75
g1 <- plot_vg(ind_vg) +
    theme(legend.position = "none")
...
ind_vg <- table_combined_de$ResultDiffResDisp %in% c("Naive+") &
  ...
  pmax(table_combined_de$ExpPropNaive, table_combined_de$ExpPropActive) > 0.75
g2 <- plot_vg(ind_vg)
...
ind_vg <- table_combined_de$ResultDiffResDisp %in% c("Active+") &
  ...
  pmax(table_combined_de$ExpPropNaive, table_combined_de$ExpPropActive) > 0.50
g4 <- plot_vg(ind_vg)
