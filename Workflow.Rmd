...
gene expression and, among other cases, has been reported to increase prior 
to cell fate decisions [@Mojtahedi2016] as well as during ageing 
...
Furthermore, the level of variability observed may differ from gene to gene;
there is evidence that some genes exhibit stable expression levels
across cell types and species [@Lin2019].
...
of the gene sets identified in downstream analyses, for example by performing
...
rownames(rowdata) <- rownames(sce_naive_active)
# Check to see that the order is correct after merge
stopifnot(all(rowdata$ensembl_gene_id == rownames(rowdata)))
...
`r Biocpkg("BASiCS")` workflow. We use it here only as part of the exploratory 
data analysis. Moreover, count-based models for dimensionality reduction (e.g. [@Townes2019;@Lopez2018]) could be used as an alternative to PCA, 
...
the joint prior introduced by Eling *et al.* leads to more stable estimation,
...
that the empirical Bayes framework [@Casella1985]
...
However, for large and less sparse datasets, a lower number of 
iterations may be sufficient. 
Practical guidance about the diagnostic criteria required to assess the 
...
## We fix a random seed here for exact reproducibility, though this isn't
## strictly necessary
set.seed(42)
prior_param_naive <- BASiCS_PriorParam(sce_naive, PriorMu = "EmpiricalBayes")
...
prior_param_active <- BASiCS_PriorParam(sce_active, PriorMu = "EmpiricalBayes")
...
if (!file.exists("rds/chain_naive.Rds")) {
  download.file(
    paste0(
      ...
      "raw/master/rds/chain_naive.Rds"
    ),
...
if (!file.exists("rds/chain_active.Rds")) {
  download.file(
    paste0(
      "https://git.ecdf.ed.ac.uk/",
      "vallejosgroup/basicsworkflow2020/",
      "raw/master/rds/chain_active.Rds"
    ),
...
If convergence has been achieved, each parameter should not evolve
...
value for `N` and `Burn` generally leads to consistent estimates across 
multiple MCMC runs.
...
TODO: why are there red lines at +-3? What do they signify?
...
Here, we compare these approaches, focusing on naive CD4^+^ T cells. We
note that repeating this analysis for active cells produces similar results. 
...
`r Biocpkg("BASiCS")`; such a workflow is described by @Maksimovic2020.
...
Figure \@ref(fig:heatmap-diffexp) shows 15 genes for each group. 
Fpr example, amongst the non DE group, we observe several genes encoding
...
library("tidyr")
ind_vg <- abs(TableCombined$MeanLog2FC) < 0.025 &
