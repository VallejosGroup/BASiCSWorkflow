...
naive.GO <- DataFrame(GO_wall[ind_signif, ])
...
# Low ESS for mu
ess_naive.mu <- coda::effectiveSize(
  coda::mcmc(displayChainBASiCS(MCMC_naive, "mu"))
)
ind_ess_naive.mu <- ess_naive.mu > 100
ess_active.mu <- coda::effectiveSize(
  coda::mcmc(displayChainBASiCS(MCMC_active, "mu"))
)
ind_ess_active.mu <- ess_naive.mu > 100

# Low ESS for delta
ess_naive.epsilon <- coda::effectiveSize(
  coda::mcmc(displayChainBASiCS(MCMC_naive, "epsilon"))
)
ind_ess_naive.epsilon <- ess_naive.mu > 100
ess_active.epsilon <- coda::effectiveSize(
  coda::mcmc(displayChainBASiCS(MCMC_active, "epsilon"))
)
ind_ess_active.epsilon <- ess_naive.mu > 100

# Genes to exclude
genes_select <- low_expr & ind_ess_naive.mu &
  ind_ess_active.mu & ind_ess_naive.epsilon & ind_ess_active.epsilon
```
...
if (!file.exists("rawCounts.tsv")) {
  download.file(
    "https://www.ebi.ac.uk/arrayexpress/files/E-MTAB-6153/E-MTAB-6153.processed.2.zip",
    destfile = "downloads/rawCounts.zip"
  )
  unzip(zipfile = "rawCounts.zip", exdir = "downloads")
...
droplet_sce <- normalize(droplet_sce)
```
...
plotReducedDim(droplet_sce, use_dimred = "PCA", colour_by = "subCellType") +
...
PSM_MCMC <- readRDS(
  file = url("https://git.ecdf.ed.ac.uk/vallejosgroup/basicsworkflow2020/raw/master/MCMCs/PSM_MCMC.rds")
)
SM_MCMC <- readRDS(
  file = url("https://git.ecdf.ed.ac.uk/vallejosgroup/basicsworkflow2020/raw/master/MCMCs/SM_MCMC.rds")
)
```
...
droplet_test <- BASiCS_TestDE(PSM_MCMC, SM_MCMC,
  EpsilonM = 1,
