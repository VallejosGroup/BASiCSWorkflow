...
plot_vg <- function(ind_vg) {
  var_genes <- table_combined_genes$GeneName[ind_vg]
...
ind_vg <- table_combined_genes$ResultDiffResDisp %in% c("Naive+") &
  ...
  (table_combined_genes$ExpPropNaive + table_combined_genes$ExpPropActive) > 1
...
ind_vg <- table_combined_genes$ResultDiffResDisp %in% c("Naive+") &
  table_combined_genes$ResultDiffMean %in% c("NoDiff") &
  abs(table_combined_genes$ExpPropNaive - table_combined_genes$ExpPropActive) > 0.45 &
  (table_combined_genes$ExpPropNaive + table_combined_genes$ExpPropActive) > 1.1
g2 <- plot_vg(ind_vg)
...
ind_vg <- table_combined_genes$ResultDiffResDisp %in% c("Active+") &
  table_combined_genes$ResultDiffMean %in% c("NoDiff") &
  abs(table_combined_genes$ExpPropNaive - table_combined_genes$ExpPropActive) < 0.01 &
  (table_combined_genes$ExpPropNaive + table_combined_genes$ExpPropActive) > 1.4
g3 <- plot_vg(ind_vg) +
    theme(legend.position = "none")
...
## i.e., genes with more bursting pattern of expression
ind_vg <- table_combined_genes$ResultDiffResDisp %in% c("Active+") &
  abs(table_combined_genes$MeanLog2FC) < 1.2 &
  abs(table_combined_genes$ExpPropNaive - table_combined_genes$ExpPropActive) > 0.2 &
  (table_combined_genes$ExpPropNaive + table_combined_genes$ExpPropActive) > 1
g4 <- plot_vg(ind_vg)
