...
Bioconductor package ecosystem (e.g. [@Lun2016; @Kim2019]), providing . 
...
Following cell-specific QC, we also recommend to remove lowly expressed genes 
for which variability estimates are less reliable. 
Here, we remove all genes that are not detected in at least 5 cells across both 
experimental conditions or for which their average read count (across all cells) 
is below 1. 
...
Subsequently we also remove spike-in molecules that were not captured
through sequencing. 

```{r spike-ins-present}
# Find which spike-ins were captured through sequencing
detected_spikes <- rowSums(assay(altExp(sce_naive_active, "spike-ins")) > 0) > 0
## Remove spike-ins that are not present from matrix and SCE object
altExp(sce_naive_active, "spike-ins") <- altExp(sce_naive_active)[detected_spikes, ]
```
...
`r Biocpkg("BASiCS")`. Firstly, if multiple batches of sequenced cells are 
...
that were added to each well (this is not required when `WithSpikes = FALSE` in
`BASiCS_MCMC`). 
...
For the CD4^+^ T cell data, the authors added a 1:50,000 dilution of the ERCC spike-in mix 1. Moreover, a volume of $9nl$ was added into each well (see [https://www.fluidigm.com/faq/ifc-9](https://www.fluidigm.com/faq/ifc-9)) 
and input concentrations $C_i$ can be downloaded from [https://assets.thermofisher.com/TFS-Assets/LSG/manuals/cms_095046.txt](https://assets.thermofisher.com/TFS-Assets/LSG/manuals/cms_095046.txt)). 
...
To update the `sce_naive` and `sce_active` objects with this information, the 
user to create `data.frame` whose first column contains the labels associated to 
...
`altExp(sce_naive)` and `altExp(sce_active)`, respectively). 
...
# Added as metadata information to each SingleCellExperiment object
metadata(sce_naive)$SpikeInput <- SpikeInput
metadata(sce_active)$SpikeInput <- SpikeInput
```

## BASiCS analysis - running the MCMC

<!--- This text in this section has not been edited; 
mostly focusing on structural changes for now --->
...
if (!file.exists("rds/MCMC_naive.rds")) {
  download.file(
    "https://git.ecdf.ed.ac.uk/vallejosgroup/basicsworkflow2020/raw/master/MCMC_naive.rds",
    destfile = "rds/MCMC_naive.rds"
  )  
}
if (!file.exists("rds/MCMC_active.rds")) {
  download.file(
    "https://git.ecdf.ed.ac.uk/vallejosgroup/basicsworkflow2020/raw/master/MCMC_active.rds",
...
dim(displayChainBASiCS(MCMC_active, Param = "mu"))
```
...
<!--- This text in this section has not been edited; 
mostly focusing on structural changes for now;
needs to include active cells too --->
