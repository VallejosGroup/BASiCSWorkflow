...
dependencies are provided as a Docker [@Boettiger2014] image at: 
...
endogeneous counts (Figure \@ref(fig:ERCC-endogeneous)).
...
```{r ERCC-endogeneous, fig.height=3, fig.width=3, fig.cap="The number of spike-in counts per cells is plotted against the number of endogeneous counts per cell. A dashed line indicates the minimum level of endogenous counts, below which cells are removed.", fig.pos='h', eval = FALSE}
...
p_stimulus <- plotColData(sce_naive_active, x = "sum", y = "detected", 
  ...
  xlab("Total number of reads (excludes non-mapped and intronic reads)") +
  ylab("Number of detected genes") +
  ggtitle("Coloured according to experimental condition")

p_batch <- plotColData(sce_naive_active, x = "sum", y = "detected", 
                       colour_by = "Individuals") +
  xlab("Total number of reads (excludes non-mapped and intronic reads)") +
  ylab("Number of detected genes") +
  ggtitle("Coloured according to the source animal (batch)")
...
<!--- CV: this text has not been edited ---> 
#### Gene-level QC. 
...
```{r gene-selection}
# Remove genes
ind_expressed <- rowSums(counts(sce_naive_active) > 0) >= 2 &
