...
values and a 
sufficient value for `N` and `Burn`, convergence is consistent.
In this instance, we focus on the diagnostic criterion proposed by 
...
indicating that the stationary distribution has not been reached by the end of
burn-in.
...
be increased, and possibly to perform more stringent quality control
on the input data.
As `r Biocpkg("BASiCS")` infers thousands of parameters, it is impractical
to assess these diagnostics individually for each parameter.

```{r, fig.cap = "Histogram of Geweke diagnostic for mean expression across all genes in naive cells. Large absolute values for this diagnostic indicate that the sampler may not have reached the stationary distribution by the end of the burn-in period."}
library("coda")
library("ggplot2")
...
```{r, fig.cap="Geweke diagnostic criterion for mean expression plotted against mean expression in naive cells."}
ggplot() +
  aes(
  ...
  geom_hex() +
  labs(x = "Mean expression", y = "Geweke diagnostic for mean expression") +
  geom_hline(yintercept = c(-3, 3), col = "firebrick", linetype = "dashed") +
  scale_fill_viridis()
```


```{r, fig.cap="Geweke diagnostic criterion for mean expression plotted against effective sample size in naive cells."}
ggplot() +
  aes(
    ess_naive,
    gew_mu_naive
...
```{r, fig.cap="Geweke diagnostic criterion for mean expression plotted against the number of detected genes per cell in naive cells."}
ggplot() +
  aes(
  ...
  geom_hex() +
  labs(x = "Number of detected genes", y = "Geweke diagnostic for mean expression") +
  geom_hline(yintercept = c(-3, 3), col = "firebrick", linetype = "dashed") +
  scale_fill_viridis()
```  
...
Therefore, we provide graphical summaries based on the ESS. It is also useful
to examine, in a similar way, other diagnostics, such as the Geweke diagnostic.
...
parameter, $\epsilon$. These values can be
obtained by running `displayChainBASiCS(chain_naive, Param = "epsilon")`.
...
The same is true for the over-dispersion parameters estimated by 
`r Biocpkg("BASiCS")`, as
...
with CV^2^. Recently, we extended `r Biocpkg("BASiCS")` to avoid the 
mean-variability 
