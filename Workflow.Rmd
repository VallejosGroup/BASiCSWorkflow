...
For example, as described below, instead of modelling expression patterns for 
each gene separately we pool information across genes with similar expression 
levels to obtain more reliable parameter estimates. The latter is particularly 
helpful for lowly expressed genes and sparse datasets, where the data contains 
less information. 
...
these parameters. For genes with similar mean expression ($\mu_i$), this prior
helps to shrink over-dispersion parameters $\delta_i$ towards similar values. 
This information-sharing approach is conceptually similar to that performed by 
@Love2014 and others, where [WIP]


sparse data is pooled to provide more reliable estimates.
