...
changes along processes that evolve over time, such as development and 
differentiation. <!--- and that can be characterised 
...
This variability can be due to deterministic or stochastic events that regulate
gene expression and, among others, has been reported to increase prior to cell
fate decisions [@Mojtahedi2016] as well as throughout 
ageing [@Martinez-jimenez2017]. 
...
dependencies are provided as a Docker [@Boettiger2015] image at: 
[https://hub.docker.com/repository/docker/alanocallaghan/bocker].
...
```{r overview, fig.cap = 'Graphical overview for the scRNA-seq analysis workflow described in this manuscript. Starting from a SingleCellExperiment object, we use the scater and scran Bioconductor packages to perform quality control and initial exploratory analyses. The primary focus of this workflow is to robustly quantify transcriptional heterogeneity within seemingly homogeneous cell populations. For this purpose, we apply the BASiCS Bioconductor package, illustrating how BASiCS can be used to analyse a single or multiple pre-specified groups of cells.', echo=FALSE}
...
`SingleCellExperiment` object which can store scRNA-seq data and its associated 
...
The `OSCA` online book each provide an extensive overview on important aspects 
of how to perform QC of scRNA-seq data, including standard exploratory analyses 
...
standard QC metrics for each cell (e.g. percentage of mitochondrial reads) and 
gene (e.g. percentage of zeroes across all cells), respectively. 
...
It can perform *global scaling* data normalisation, calculating cell-specific 
...
be assessed using `BASiCS_EffectiveSize` and `BASiCS_DiagPlot`. 
...
<!---`BASiCS_MCMC` can be run using four different major settings (Table 1). 
...
improves posterior estimates for small sample sizes and 
...
identifiers. In order to facilitate interpretation, it is often useful
to generate a mapping from Ensembl gene IDs to 
gene symbols using 
In order to perform GO enrichment analysis, and to aid in the biological 
...
using the `r Biocpkg("biomaRt")` [@Durinck2009].
...
- **R packages**: BASiCS `r packageVersion("BASiCS")`, scran 
`r packageVersion("scran")`, scater `r packageVersion("scater")`. Version 
...
For the first case study, we use scRNA-seq data generated for CD4^+^ T cells
...
The input matrix contains data for`r ncol(CD4_raw)` cells and `r nrow(CD4_raw)`
genes (including `sum(grepl("ERCC", rownames(CD4_raw)))` ERCC spike-ins). 
...
The data and metadata described above is then converted into a 
...
from young *Mus musculus domesticus* (hereafter also referred to as black 6; B6) 
animals, purified using MACS-based cell sorting. 
The following code is used to extract these samples from the full dataset.
...
[E-MTAB-4888](https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-4888/) has
...
(iv) more than 10% or less than 0.5% reads mapping to mitochondrial genes.
...
<!-- 
   +
  geom_vline(xintercept = min_reads, linetype = "dashed", colour = "grey60") +
  geom_vline(xintercept = max_reads, linetype = "dashed", colour = "grey60")
...
<!--- CV: not sure we need the PCA analysis here --->
...
```{r diag-hist-naive, fig.cap="Distribution of effective sample size across all genes for mu, delta and epsilon parameters. A dashed line indicates the minimum effective sample size for genes to be considered in differential expression tests."}
...
delta_ess <- BASiCS_DiagHist(MCMC_naive, Param = "delta") +
  xlab("ESS: delta") +
  geom_vline(xintercept = 100, linetype = "dashed", colour = "grey80")
epsilon_ess <- BASiCS_DiagHist(MCMC_naive, Param = "epsilon") +
  xlab("ESS: epsilon") +
  geom_vline(xintercept = 100, linetype = "dashed", colour = "grey80")
multiplot(mu_ess, delta_ess, epsilon_ess, cols = 2)
```

```{r diag-plot-naive, fig.cap="Effective sample size is plotted against parameter estimates for mu, delta and epsilon parameters. A dashed line indicates the minimum effective sample size for genes to be considered in differential expression tests. Colour indicates local density, with lighter colours representing higher density."}
...
delta_ess2 <- BASiCS_DiagPlot(MCMC_naive, Param = "delta") +
  theme(legend.position = "none") +
  geom_hline(yintercept = 100, linetype = "dashed", colour = "grey80")
epsilon_ess2 <- BASiCS_DiagPlot(MCMC_naive, Param = "epsilon") +
  theme(legend.position = "none") +
  geom_hline(yintercept = 100, linetype = "dashed", colour = "grey80")
multiplot(mu_ess2, delta_ess2, epsilon_ess2, cols = 2)
```

We can see several genes show very low sample size for mu. We can identify 
these using `BASiCS_EffectiveSize`.

```{r low-ess-genes, fig.cap="Distribution of effective sample size for the mu mean expression parameter across all genes. A dashed line indicates the minimum effective sample size for genes to be considered in differential expression tests."}
...
We will use the `r CRANpkg("ggpointdensity")` to visualise the density of
...
```{r mean-dispersion-basics, fig.cap="Epsilon residual over-dispersion estimates are plotted against mean expression."}
...
```{r ess-diag-hist, fig.cap="Distribution of effective sample size across all genes for mu, delta and epsilon parameters. A dashed line indicates the minimum effective sample size for genes to be considered in differential expression tests."}
...
delta_ess <- BASiCS_DiagHist(MCMC_active, Param = "delta") +
  ...
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
  geom_vline(xintercept = 100, linetype = "dashed", colour = "grey80")
epsilon_ess <- BASiCS_DiagHist(MCMC_active, Param = "delta") +
  xlab("ESS: mu") +
  theme(axis.text.x = element_text(hjust = 1, angle = 45)) +
  geom_vline(xintercept = 100, linetype = "dashed", colour = "grey80")
multiplot(mu_ess, delta_ess, epsilon_ess, cols = 2)
...
```{r ess-diag-plot, fig.cap="Effective sample size is plotted against parameter estimates for mu, delta and epsilon parameters. A dashed line indicates the minimum effective sample size for genes to be considered in differential expression tests. Colour indicates local density, with lighter colours representing higher density."}
...
delta_ess2 <- BASiCS_DiagPlot(MCMC_active, Param = "delta") +
  theme(legend.position = "none") +
  geom_hline(yintercept = 100, linetype = "dashed", colour = "grey80")
epsilon_ess2 <- BASiCS_DiagPlot(MCMC_active, Param = "epsilon") +
  theme(legend.position = "none") +
  geom_hline(yintercept = 100, linetype = "dashed", colour = "grey80")
multiplot(mu_ess2, delta_ess2, epsilon_ess2, cols = 2)
```
...
```{r active-fit, fig.cap="Over-dispersion is plotted against mean expression for all genes in active cells, on a log-log scale. Colour corresponds to local density, with lighter colours representing higher density. The curve represents the inferred trend of over-dispersion against mean, and the shaded area indicates the 95% credible interval."}
...
and `r sum(TableDisp$ResultDiffDisp=="ExcludedLowESS")`
...
naive_genes <- as.integer(TableMean$ResultDiffMean == "Naive+")
names(naive_genes) <- TableMean$GeneName
...
for (i in 1:nrow(naive_GO)) {
  ...
  genes <- as.character(intersect(
    genes,
    TableMean$GeneName[naive_genes == 1]
  ))
...
```{r go-analysis-active, eval = FALSE}
# Collect significan genes as 1 and all other as 0
active_genes <- as.integer(TableMean$ResultDiffMean == "Active+")
names(active_genes) <- TableMean$GeneName
...
for (i in 1:nrow(active_GO)) {
  ...
  genes <- as.character(intersect(
    genes,
    TableMean$GeneName[active_genes == 1]
  ))
  all_genes[i] <- paste(genenames[genes, "external_gene_name"],
    collapse = ", "
  )
...
we will now demonstrate the use of `BASiCS` on a system that shows more subtle
...
```{r droplet-sce, fig.cap="Principal component plot of presomitic and somitic mesoderm cells. Colour indicates the inferred cell type [@Ibarra-Soria2018]."}
...
```{r heatmap-diffexp, fig.cap="Heatmap of 30 differentially expressed genes in PSM and SM cells. Colour indicates expression level; colour bars on the right of heatmap segments indicate the average expression for each gene."}
library("ComplexHeatmap")
library("circlize")
...
p_ind <- order(PSM_mean$ProbDiffMean, decreasing = TRUE)[heatmap_seq]
PSM_mean_s <- PSM_mean[p_ind, ]
psm_counts_psm <- counts(PSM_Data)[PSM_mean_s$GeneName, ]
sm_counts_psm <- counts(SM_Data)[PSM_mean_s$GeneName, ]

s_ind <- order(SM_mean$ProbDiffMean, decreasing = TRUE)[heatmap_seq]
SM_mean_s <- SM_mean[s_ind, ]
psm_counts_sm <- counts(PSM_Data)[SM_mean_s$GeneName, ]
sm_counts_sm <- counts(SM_Data)[SM_mean_s$GeneName, ]

all_mean <- c(
  PSM_mean_s$Mean1, PSM_mean_s$Mean2, SM_mean_s$Mean1, SM_mean_s$Mean2
)
...
counts_psm <- rbind(psm_counts_psm, psm_counts_sm)
counts_sm <- rbind(sm_counts_psm, sm_counts_sm)
...
Heatmap(
  ...
  column_title = "SM cells",
  cluster_rows = FALSE,
  split = split,
  right_annotation = rowAnnotation(
    log_mu = log(c(PSM_mean_s$Mean2, SM_mean_s$Mean2)),
...
Heatmap(
  ...
  column_dend_height = unit(0.2, "npc"),
  row_names_gp = gpar(fontsize = fontsize),
  column_title = "PSM cells",
  column_title_side = "bottom",
  name = "log10(count + 1)",
  split = split,
  right_annotation = rowAnnotation(
    log_mu = log(c(PSM_mean_s$Mean1, SM_mean_s$Mean1)),
    col = list(log_mu = colour)
  ),
  cluster_rows = FALSE,
  col = viridis(100))
...
```{r gene-lists-disp}
# Highly expressed in somitic mesoderm
ind_sm <- Droplet_TableResDisp$ResultDiffResDisp == "SM+"
SM_resdisp <- Droplet_TableResDisp[ind_sm, ]
SM_resdisp <- SM_resdisp[order(SM_resdisp$ResDispDistance, decreasing = FALSE), ]
SM_resdisp$Symbol <- genenames[SM_resdisp$GeneName, 2]

# Highly expressed in pre-somitic mesoderm
ind_psm <- Droplet_TableResDisp$ResultDiffResDisp == "PSM+"
PSM_resdisp <- Droplet_TableResDisp[ind_psm, ]
PSM_resdisp <- PSM_resdisp[order(PSM_resdisp$ResDispDistance, decreasing = TRUE), ]
PSM_resdisp$Symbol <- genenames[PSM_resdisp$GeneName, 2]
```


```{r heatmap-diffresdisp, fig.cap="Heatmap of 30 genes with higher residual over-dispersion in PSM or SM cells. Colour indicates expression level; colour bars on the right of heatmap segments indicate the residual over-dispersion for each gene."}
heatmap_ngenes <- 15
heatmap_seq <- seq_len(heatmap_ngenes)
p_ind <- order(PSM_resdisp$ProbDiffResDisp, decreasing = TRUE)[heatmap_seq]
PSM_resdisp_s <- PSM_resdisp[p_ind, ]
psm_counts_psm <- counts(PSM_Data)[PSM_resdisp_s$GeneName, ]
sm_counts_psm <- counts(SM_Data)[PSM_resdisp_s$GeneName, ]

s_ind <- order(SM_resdisp$ProbDiffResDisp, decreasing = TRUE)[heatmap_seq]
SM_resdisp_s <- SM_resdisp[s_ind, ]
psm_counts_sm <- counts(PSM_Data)[SM_resdisp_s$GeneName, ]
sm_counts_sm <- counts(SM_Data)[SM_resdisp_s$GeneName, ]

# all_resdisp <- c(PSM_resdisp_s$ResDisp1, PSM_resdisp_s$ResDisp2,
#   SM_resdisp_s$ResDisp1, SM_resdisp_s$ResDisp2)
...
Heatmap(
  ...
  split = split,
  right_annotation = rowAnnotation(
    epsilon = c(PSM_resdisp_s$ResDisp2, SM_resdisp_s$ResDisp2),
...
Heatmap(
  ...
  split = split,
  right_annotation = rowAnnotation(
    epsilon = c(PSM_resdisp_s$ResDisp1, SM_resdisp_s$ResDisp1),
    col = list(epsilon = colour)
  ),
  cluster_rows = FALSE,
  col = viridis(100))
