...
due to deterministic or stochastic events that regulate gene expression and,
among others, has been reported to increase prior to cell phate decisions 
[@Mojtahedi2016] as well as throughout ageing [@Martinez-jimenez2017]. 
...
ecosystem to analyse scRNA-seq data [@Lun2016; @Kim2019]. 
We present a step-by-step workflow which combines the `r Biocpkg("scater")` and 
`r Biocpkg("scran")` packages to perform quality control steps as well as 
low-level exploratory analyses [@McCarthy2017; @Lun2016]. 
Our main focus is to robustly quantify transcriptional variability within and
between pre-specified cell populations, such as experimental conditions or
cell types.
For this purpose, we use `r Biocpkg("BASiCS")` [@Vallejos2015; @Vallejos2016;
...
to highlight *lowly variable genes* (LVGs) that exhibit stable expression 
...
If the aim is to compare expression patterns between two or more pre-specified 
...
Whilst several differential expression tools have been proposed in the context 
of scRNA-seq (e.g. [@Kharchenko2014; @Finak2015]), some evidence suggests that 
these do not generally outperform popular bulk RNA-seq tools [@Soneson2018]. 
Moreover, most of these methods are unable to uncover the complex 
...
In this manuscript, we briefly discuss the different sources of variability that 
arise in scRNA-seq datasets and some of the strategies that have been 
designed to control or attenuate technical noise in scRNA-seq protocols. 
...
at the level of individual genes [@Elowitz2002]. 
