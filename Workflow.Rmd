...
The focus of this article is on more subtle variability that can 
...
In particular, it can perform *global scaling* data normalisation, calculating 
...
components (see the `decomposeVar` function). 
...
algorithm [@Roberts2009].
This is implemented in the `BASiCS_MCMC` function, which can be run using four 
different major settings (see Table 1). 
If spike-in molecules and the user sets `WithSpikes = TRUE` (default), the
model uses this information in order to infer technical noise.
Alternatively, if `WithSpikes = FALSE` [TEXT TBC].
We recommend users to select `Regression = TRUE` as a default option, as it
[TEXT TBC].
