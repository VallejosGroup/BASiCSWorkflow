...
gene expression and, among other cases, has been seen to increase prior to 
...
We also use the visualisation tools implemented in `r Biocpkg("scater")` to 
...
Ensembl gene IDs to gene symbols using the BioMart suite ([http://www.biomart.org](http://www.biomart.org)) via the 
`r Biocpkg("biomaRt")` package [@Durinck2009].
...
# Check if  order is correct after merge; add to he SingleCellExperiment object
...
`r sum(rowData(sce_naive_active)$gene_biotype == "protein_coding")` protein 
coding genes that are contained in the data. These are selected below. 

```{r}
pcg <- which(rowData(sce_naive_active)$gene_biotype == "protein_coding")
sce_naive_active <- sce_naive_active[pcg, ]
```
...
scaling normalisation using `r Biocpkg("scran")` and a 
...
to obtain.
Here, we remove genes that are not detected in at least 10 cells across all 
cells or that have an average read count below 1. 
These thresholds are generally arbitrary, can vary across datasets and should be 
...
rowData(sce_naive_active)$include_gene <- rowData(sce_naive_active)$mean >= 1 &
  rowData(sce_naive_active)$detected_cells >= 20
plotRowData(
    sce_naive_active,
    x = "detected_cells",
    y = "mean",
  ...
  scale_y_log10() +
  theme(legend.position = "bottom") +
geom_vline(xintercept = 10, linetype = "dashed", col = "grey60") +
...
However, less iterations may be sufficient for large and less sparse datasets. 
...
## We fix a random seed for exact reproducibility, but this is not strictly required
set.seed(42)
...
website <- "https://git.ecdf.ed.ac.uk/vallejosgroup/basicsworkflow2020/raw/master/rds/"
if (!file.exists("rds/chain_naive.Rds")) {
  download.file(
    paste0(website, "chain_naive.Rds"),
    destfile = "rds/chain_naive.Rds"
  )  
}
if (!file.exists("rds/chain_active.Rds")) {
  download.file(
    paste0(website, "chain_active.Rds"),
...
Before interpreting the estimates obtained by `r Biocpkg("BASiCS")`, 
...
value for `N` and `Burn` generally leads to largely consistent estimates across 
multiple MCMC runs.
...
Here, we illustrate usage for two of such metrics focusing using the MCMC
...
a threshold at ESS $< 100$ is applied).
...
$\epsilon_i$ is below a pre-specified threshold `EpsilonThreshold`. 
...
In some circumstances, however, it may be of interest to rank genes and to select the
...
ind_hvg <- which(
  hvg_table$HVG &
    log10(hvg_table$Mu) > low_exp &
    log10(hvg_table$Mu) < up_exp
)
hvg1 <- hvg_table[ind_hvg, ]
ind_lvg <- which(
  lvg_table$LVG &
    log10(lvg_table$Mu) > low_exp &
    log10(lvg_table$Mu) < up_exp
)
lvg1 <- lvg_table[ind_lvg, ]
...
top_hvg <- log10(t(dc_naive[hvg1$GeneName[1:3], ]) + 1)
top_lvg <- log10(t(dc_naive[lvg1$GeneName[1:3], ]) + 1)
# Add genenames
colnames(top_hvg) <- hvg1$external_gene_name[1:3]
colnames(top_lvg) <- lvg1$external_gene_name[1:3]
...
plot_hvg <- ggplot(melt(top_hvg), aes(x = Var2, y = value)) + 
  plot_params + ggtitle("Example HVGs")

plot_lvg <- ggplot(melt(top_lvg), aes(x = Var2, y = value)) + 
...
Differential expression testing is provided via the
...
table_de_mean <- as.data.frame(test_de, Parameter = "Mean", Filter = FALSE)
table_de_resdisp <- as.data.frame(test_de, Parameter = "ResDisp", Filter = FALSE)
```
...
biologically meaningful patterns among these genes. 
For example, this could be performed using `r Biocpkg("goseq")` [@Young2010].
We do not perform this here, but a relevant workflow is described by @Maksimovic2020.

We first focus on the differential mean expression test. 
...
`BASiCS_TestDE`). 
Among other causes, the latter can be due to overall differences in mRNA content
between the groups. 
...
col <- colorRamp2(
  breaks = seq(0, log10(max(c(counts_naive, counts_active) + 1)), length.out = 20),
  colors = viridis(20)
)
mu_col <- colorRamp2(
  breaks = seq(0, log10(max(c(active_mean$Mean1, naive_mean$Mean1, nonde_mean$Mean1))), length.out = 20),
...
Heatmap(
  ...
  right_annotation = rowAnnotation(
    log_mu = log10(c(active_mean$Mean1, naive_mean$Mean1, nonde_mean$Mean1)),
  ...
  Heatmap(
    ...
    right_annotation = rowAnnotation(
      log_mu = log10(c(active_mean$Mean2, naive_mean$Mean2, nonde_mean$Mean2)),
      col = list(log_mu = mu_col)
    ),
    cluster_rows = FALSE,
    col = col)
...
```{r, var-plots, fig.width = 7, fig.height = 5, fig.cap="A, C: log2 change in expression against against log mean expression for genes with higher residual over-dispersion in naive (A) cells and active (B) cells. B, D: Proportion of expressed cells for genes, with higher residual over-dispersion in naive cells (B) and active (C) cells."}
...
## why are we printing dims here?
dim(gene_tests)
dim(table_de_combined)
...
g1 <- ggplot(table_combined_de[table_combined_de$ResultDiffResDisp == "Naive+", ]) +
  aes(log10(MeanOverall), MeanLog2FC) +
  plot_params +
  ylim(-15, 15) +
  labs(x = "log10 mean expression", y = "log2 fold change") +
  geom_abline(slope = 0, intercept = 0, colour = "firebrick", linetype = "dashed")

g2 <- ggplot(table_combined_de[table_combined_de$ResultDiffResDisp == "Active+", ]) +
  aes(log10(MeanOverall), MeanLog2FC) +
  plot_params +
  ylim(-15, 15) +
  labs(x = "log10 mean expression", y = "log2 fold change") +
...
g3 <- ggplot(table_combined_de[table_combined_de$ResultDiffResDisp == "Naive+", ]) +
  aes(x = ExpPropNaive, y = ExpPropActive) +
  plot_params +
  labs(
    x = "Proportion of cells expressing (naive)",
    y = "Proportion of cells \nexpressing (active)"
  ) +
  geom_abline(slope = 1, intercept = 0, colour = "firebrick", linetype = "dashed")

g4 <- ggplot(table_combined_de[table_combined_de$ResultDiffResDisp == "Active+", ]) +
  aes(x = ExpPropNaive, y = ExpPropActive) +
  plot_params +
  labs(
    x = "Proportion of cells expressing (naive)",
...
```{r, props, fig.width = 7, fig.height = 5, fig.cap="Proportion of expression in naive and active cells."}
# DiffResDisp is not confounded by mean expression changes
# However, we do observe a change in terms of the proportion of cells with expression
# Higher var <-> lower proportion of expression

g5 <- ggplot(table_combined_de) +
  aes(x = ExpPropNaive, y = ExpPropActive) +
  plot_params +
  labs(
    x = "Proportion of cells expressing (naive)",
    y = "Proportion of cells expressing (active)"
  ) +
  geom_abline(slope = 1, intercept = 0, colour = "firebrick", linetype = "dashed")
g5
```
...
```{r, violin-diffresdisp-naive-similar, fig.width = 7, fig.height = 5, fig.cap="Violin plot of denoised counts for three genes with higher residual over-dispersion in naive cells, and similar levels of detection in active and naive populations."}
table_combined_de <- table_combined_de[order(abs(table_combined_de$ResDispDistance), decreasing = TRUE),]
...
```{r, violin-diffresdisp-naive-dropout, fig.width = 7, fig.height = 5, fig.cap="Violin plot of denoised counts for eleven genes with higher residual over-dispersion in naive cells and different levels of detection in naive and active cells."}
# genes more variable in naive     &
# not differentially expressed     &
# with different levels of dropout &
# expressed in c. 50% of cells
## ie, genes with much more bursting pattern of expression
# nuclear genes, so maybe not required as often when not active
ind_vg <- table_combined_de$ResultDiffResDisp %in% c("Naive+") & 
  table_combined_de$ResultDiffMean %in% c("NoDiff") &
  abs(table_combined_de$ExpPropNaive - table_combined_de$ExpPropActive) > 0.40 &
  (table_combined_de$ExpPropNaive + table_combined_de$ExpPropActive) > 1
plot_vg(ind_vg)
```

```{r, violin-diffresdisp-active-similar, fig.width = 7, fig.height = 5, fig.cap="Violin plot of denoised counts for twelve genes with higher residual over-dispersion in active cells and similar levels of detection in naive and active cells."}
# genes more variable in Active   &
# not differentially expressed   &
# with similar levels of dropout &
# expressed in c. 50% of cells
## ie, genes that are probably unimodal but with higher variance
# dock10/dock11 - maybe bimodal with varying cell composition
# MORF4L1 (Mortality Factor 4 Like 1) is a Protein Coding gene. Among its related pathways are Chromatin organization and Chromatin Regulation / Acetylation. Gene Ontology (GO) annotations related to this gene include chromatin binding and protein N-terminus binding. An important paralog of this gene is MORF4L2.
ind_vg <- table_combined_de$ResultDiffResDisp %in% c("Active+") & 
  table_combined_de$ResultDiffMean %in% c("NoDiff") &
  abs(table_combined_de$ExpPropNaive - table_combined_de$ExpPropActive) < 0.2 &
...
```{r, violin-diffresdisp-active-dropout, fig.width = 7, fig.height = 5, fig.cap="Violin plot of denoised counts for six genes with higher residual over-dispersion in active cells and different levels of detection in naive and active cells."}
# genes more variable in Active     &
# not differentially expressed     &
# with different levels of dropout &
# expressed in c. 50% of cells
## ie, genes with much more bursting pattern of expression????
## actually seems to be mostly just higher variability
ind_vg <- table_combined_de$ResultDiffResDisp %in% c("Active+") & 
  # table_combined_de$ResultDiffMean %in% c("NoDiff") &
  abs(table_combined_de$MeanLog2FC) < 1 &
  abs(table_combined_de$ExpPropNaive - table_combined_de$ExpPropActive) > 0.20 &
  (table_combined_de$ExpPropNaive + table_combined_de$ExpPropActive) > 1
plot_vg(ind_vg)
```
...
Figure \@ref(fig:violin-diffresdisp-naive-similar) shows denoised counts for
five genes with higher residual over-dispersion in naive cells. Here, we show only genes with 
similar mean expression.



```{r, eval=FALSE, echo=FALSE}
# ## what are these plots meant to show?
# for(i in 1:length(var_genes)) {
#   plot(sort(var_naive[i, ]), pch = 16, main = i)
#   points(sort(var_active[i, ]), col = "red", pch = 16)
# }

# ## what are these plots meant to show?
# for(i in 1:length(var_genes)) {
#   par(mfrow = c(1,2))
#   plot(log10(var_naive[i, var_naive[i, ] > 0]), main = i)
#   plot(log10(var_active[i, var_active[i, ] > 0]), main = i)
# }
...
#   table_combined_de$ResultDiffMean %in% c("NoDiff") &
#   abs(table_combined_de$ExpPropNaive - table_combined_de$ExpPropActive) < 0.10 &
#   (table_combined_de$ExpPropNaive + table_combined_de$ExpPropActive) / 2 > 0.5
...
# for(i in 1:length(var_genes)) {
#   par(mfrow = c(1,2))
#   boxplot(log10(var_naive[i, var_naive[i, ] > 0]), main = i)
#   boxplot(log10(var_active[i, var_active[i, ] > 0]), main = i)
# }
# ## what are these plots meant to show?
# for(i in 1:length(var_genes)) {
#   par(mfrow = c(1,2))
...
```{r violin-diffresdisp2, eval=FALSE, echo=FALSE}
## fig.width = 7, fig.height = 5, fig.cap="Violin plot of denoised counts for five genes with higher residual over-dispersion in naive cells."
...
#table_de_combined <- table_de_combined[order(table_de_combined$ProbDiffResDisp, decreasing = TRUE),]
#ind_vg <- #abs(table_de_combined$MeanLog2FC) < 0.5 &
#  log10(table_de_combined$MeanOverall) > 2 &
#  log10(table_de_combined$MeanOverall) < 2.5  &
#  abs(table_de_combined$ResDispDistance) > 2 &
#  table_de_combined$ResultDiffMean %in% c("NoDiff") &
#  table_de_combined$ResultDiffResDisp %in% c("Naive+")
#table(ind_vg)
#var_genes <- table_de_combined$GeneName[ind_vg]
#var_genes <- var_genes[1:3]
...
# ind_vg <- abs(table_de_combined$MeanLog2FC) < 0.1 &
#   # log10(table_de_combined$MeanOverall) > 2 &
#   # log10(table_de_combined$MeanOverall) < 2.5 &
#   table_de_combined$ResultDiffMean %in% c("NoDiff") &
#   table_de_combined$ResultDiffResDisp %in% c("Active+")
...
if (!file.exists("rds/chain_naive_nospikes.Rds")) {
  download.file(
    paste0(
      ...
      "raw/master/rds/chain_naive_nospikes.Rds"
    ),
...
These genes each have zero read counts across the entire naive population.
...
very low mean expression level as inferred without spike-in genes, comprising
...
ggplot() +
  aes(colMedians(mu_spikes), colMedians(mu_nospikes)) +
  geom_pointdensity() +
  scale_colour_viridis() +
  scale_x_log10() +
  scale_y_log10() +
  geom_abline(colour = "firebrick", linetype = "dashed", slope = 1, intercept = 0) +
  labs(
...
on EBI ArrayExpress under accession number E-MTAB-4888.
