...
Note that cluster labels are not available for 
`r sum(is.na(cluster_labels$cellType))` cells. 
...
Note that the authors identified some sub-structure within pre-somitic mesoderm 
cells
...
For further processing steps, we remove lowly expressed genes captured in less
than 10\% of cells. 
...
After this filtering step, `r nrow(droplet_sce)` remain in the data. 

To visualise possible sub-structure in the data, we normalise both cell 
populations using the `scran` package. Log-normalised counts ($\log(x+1)$) 
will be used for visualisation purposes. 

```{r norm-drop}
# Calculate scran size factors
droplet_sce <- computeSumFactors(droplet_sce,
  clusters = colData(droplet_sce)$subCellType
)
# Calculate log-normalised expression counts
...
Note: by default, `runPCA()` uses log-normalised counts as input.
