...
In `r Biocpkg("scran")`, the squared coefficient of variation (CV^2^) is used 
as a proxy for transcriptional variability within a population of cells
Instead, `r Biocpkg("BASiCS")` uses gene-specific over-dispersion 
parameters $\delta_i$ for this purpose. 
As seen in Fig .. these estimates are highly correlated. 
Both variability measures are confounded by differences in mean expression, e.g.
highly expressed genes tend to exhibit lower variability according to these
metrics. For example, Figure \@ref(fig:dispersion-vs-mean)A displays 
`r Biocpkg("BASiCS")` posterior estimates for over-dispersion parameters 
inferred for the naive CD4^+^ T cells and how these relate to mean expression 
estimates (the `BASiCS_ShowFit` function can be used to generate such a plot, 
but we will generate it manually to demonstrate how users can extract this 
information from a `BASiCS_MCMC` object. 
A similar behaviour is observed for `r Biocpkg("scran")` estimates (see Figure \@ref(fig:dispersion-vs-mean)B). 
...
g2 <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(log(mean_scran), log(cv2_scran))) +
  scale_colour_viridis() +
  theme(
    legend.position = "bottom",
    legend.text = element_text(angle = 45, hjust = 1)
...
g3 <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(log(cv2_scran), log(delta))) +
  scale_colour_viridis() +
  theme(
    legend.position = "bottom",
    legend.text = element_text(angle = 45, hjust = 1)
...
g6 <- ggplot(parameter_df[ind_not_na, ]) +
  geom_pointdensity(aes(DM, epsilon)) +
  scale_colour_viridis() +
  theme(
    legend.position = "bottom",
    legend.text = element_text(angle = 45, hjust = 1)
