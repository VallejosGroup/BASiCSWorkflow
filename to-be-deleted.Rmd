...
As seen in Figures \@ref(fig:dispersion-vs-mean)A and 
\@ref(fig:dispersion-vs-mean)B, both variability measures are confounded by 
differences in mean expression, e.g. highly expressed genes tend to exhibit 
lower variability according to these metrics (note that the `BASiCS_ShowFit` 
function can be used to explore this relationship, but we generated the plot 
manually to demonstrate how users can extract this information from a 
`BASiCS_MCMC` object). 
