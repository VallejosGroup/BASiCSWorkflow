...
`BASiCS_DetectLVG` to identify genes with substantially high (HVG) or low 
(LVG) transcriptional variability within a population of cells. 
...
HVG are marked as those for which $\epsilon_i$ exceeds a user-defined 
variability threshold with high probability, where the probability cut-off is 
chosen to match a given expected false discovery rate (EFDR; default 
EFDR = 10\%) [@Newton2004].
A similar approach is implemented for LVG detection, but it is based on whether 
$\epsilon_i$ is below a pre-specified variability threshold. 
In principle, the variability threshold could be directly specified in the 
same scale as $\epsilon_i$. 
For example, if the threshold for $\epsilon_i$ is equal to $\log(2)$, HVG would 
be those genes for which the inferred over-dispersion is at least $2$ times 
higher than would be expected for associated mean expression level. 
In practice, however, it may be of interest to rank genes according to residual 
over-dispersion. 
For example, the `BASiCS_DetectHVG` function selects the 10% most HVG when using
`PercentileThreshold = 0.9`.
Similarly, when detecting lowly variable genes, the `BASiCS_DetectLVG` selects 
the 10% most LVG by using `PercentileThreshold = 0.1`.
The results of this analysis in the context of naive CD4$^+$T cells is
displayed below. 
...
vg_plot <- ggplot(HVG@Table[!is.na(HVG@Table$Epsilon),]) +
  geom_point(aes(log(Mu), Epsilon), colour = "gray") +
  labs(
    x = "BASiCS means",
    y = "BASiCS residual\nover-dispersion"
  ) +
 ...
 # +
 # geom_point(
 #   data = HVG@Table[!is.na(HVG@Table$Epsilon) & HVG@Table$HVG == TRUE,], 
 #   aes(log(Mu), Epsilon), colour = "red") +
  #geom_point(
 #   data = LVG@Table[!is.na(LVG@Table$Epsilon) & LVG@Table$LVG == TRUE,], 
 #            aes(log(Mu), Epsilon), colour = "blue")
```
...
<!--- NEXT ANALYSIS AIMS TO INTERPRET SOME EXAMPLES ---> 
<!--- MAYBE LOOK FOR OVERLAP WITH GIGASCIENcE PAPER LIST? ---> 

```{r Denoised_naive}
DC_naive <- BASiCS_DenoisedCounts(sce_naive, chain_naive)

# Filter out lowly expressed genes
HVG_table1 <- HVG_table[log(HVG_table$Mu) > 4 & log(HVG_table$Mu) < 4.2,]
LVG_table1 <- LVG_table[log(LVG_table$Mu) > 4 & log(LVG_table$Mu) < 4.2,]

library(reshape2)
topHVG <- log10(t(DC_naive[HVG_table1$GeneName[1:3], ]) + 1)
topLVG <- log10(t(DC_naive[LVG_table1$GeneName[1:3], ]) + 1)
...
plot_lvg <- ggplot(melt(topLVG), aes(x = Var2, y = value)) + 
  geom_violin() + coord_flip() + ylim(-0.01,3.2) +
  geom_jitter(position=position_jitter(0.3))
vg_plot1 <- vg_plot + geom_vline(xintercept = c(4, 4.2), lty = 2)
plot_grid(vg_plot1, plot_hvg, plot_lvg, ncol = 3)

# Filter out lowly expressed genes
HVG_table1 <- HVG_table[log(HVG_table$Mu) > 2 & log(HVG_table$Mu) < 2.2,]
LVG_table1 <- LVG_table[log(LVG_table$Mu) > 2 & log(LVG_table$Mu) < 2.2,]
...
plot_lvg <- ggplot(melt(topLVG), aes(x = Var2, y = value)) + 
  geom_violin() + coord_flip() + ylim(-0.01,3.2) +
  geom_jitter(position=position_jitter(0.3))
vg_plot2 <- vg_plot + geom_vline(xintercept = c(2, 2.2), lty = 2)
plot_grid(vg_plot2, plot_hvg, plot_lvg, ncol = 3)

# Filter out lowly expressed genes
HVG_table1 <- HVG_table[log(HVG_table$Mu) > 1 & log(HVG_table$Mu) < 1.2,]
LVG_table1 <- LVG_table[log(LVG_table$Mu) > 1 & log(LVG_table$Mu) < 1.2,]
...
plot_lvg <- ggplot(melt(topLVG), aes(x = Var2, y = value)) + 
  geom_violin() + coord_flip() + ylim(-0.01,3.2) +
  geom_jitter(position=position_jitter(0.3))
vg_plot3 <- vg_plot + geom_vline(xintercept = c(1, 1.2), lty = 2)
plot_grid(vg_plot3, plot_hvg, plot_lvg, ncol = 3)
```
