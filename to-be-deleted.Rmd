...
those with the highest or the lowest variability. 
<!--- CV: LIKELY TO BE EDITED TO USE THE APPROACH ABOVE --->
The `PercentileThreshold` parameter can be used to define the threshold based
on the empirical distribution of posterior estimates for $\epsilon_i$. 
...
# Remove lowly expressed genes without valid epsilon estimates
HVG_Table <- HVG@Table[!is.na(HVG@Table$Epsilon),]
LVG_Table <- LVG@Table[!is.na(LVG@Table$Epsilon),]

ggplot(HVG_Table) +
  geom_point(aes(log10(Mu), Epsilon), colour = "gray") +
  xlab("BASiCS means (log10)") +
  ylab("BASiCS residual\nover-dispersion") +
  geom_hline(yintercept = 0, lty = 2) +
  geom_point(data = HVG_Table[HVG_Table$HVG == TRUE, ],
             aes(log10(Mu), Epsilon), colour = "red") +
  geom_point(data = LVG_Table[LVG_Table$LVG == TRUE, ],
             aes(log10(Mu), Epsilon), colour = "blue")
```

This analysis results in the detection of *`r sum(HVG_table$HVG, na.rm=TRUE)`*
HVG and *`r sum(LVG_table$LVG, na.rm=TRUE)`* LVG.
As an illustration, Figure \@ref(fig:plot_example_vg_naive) shows the 
...
\@ref(fig:plot_example_vg_naive)A). 
As expected HVG tend to exhibit a wider distribution and potentially bimodal
distribution (Figure \@ref(fig:plot_example_vg_naive)B).
Instead, LVG tend to have more narrow and unimodal distributions 
(Figure \@ref(fig:plot_example_vg_naive)C).

<!--- CV: FUNCTION IS LIKELY AN OVERKILL, BUT HELPFUL TO EXPLORE EXAMPLES ---> 
<!--- CV: EDIT FIGURE CAPTION ---> 

```{r plot_example_vg_naive}
library(reshape2)

# Obtain normalised expression values
DC_naive <- BASiCS_DenoisedCounts(sce_naive, chain_naive)

my_plot_vg <- function(DC, HVG_Table, LVG_Table, low.exp, up.exp) {
  
  # Mean vs residual over-dispersion plot for all genes
  vg_plot <- ggplot(HVG_Table) +
    geom_point(aes(log10(Mu), Epsilon), colour = "gray") +
    xlab("BASiCS means (log10)") +
    ylab("BASiCS residual\nover-dispersion") +
    geom_hline(yintercept = 0, lty = 2) +
  ...
  # Select genes with similar mean expression values
  HVG1 <- HVG_Table[log10(HVG_Table$Mu) > low.exp & log10(HVG_Table$Mu) < up.exp,]
  LVG1 <- LVG_Table[log10(LVG_Table$Mu) > low.exp & log10(LVG_Table$Mu) < up.exp,]  
  ...
  plot_hvg <- ggplot(melt(topHVG), aes(x = Var2, y = value)) + 
    geom_violin() + coord_flip() + ylim(-0.05, max(log10(DC+1))) +
...
my_plot_vg(DC_naive, HVG, LVG, low.exp = 2, up.exp = 2.2)
```
