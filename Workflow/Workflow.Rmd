...
if (!file.exists("genelength.rds")) {
  ...
  saveRDS(genelength, "genelength.rds")  
}
genelength <- readRDS("genelength.rds")
...
# Read in raw data
CD4_raw <- read.table("raw_data.txt", header = TRUE, sep = "\t")
...
CD4_metadata <- read.table("metadata_file.txt", header = TRUE, sep = "\t")
...
file <- "cms_095046.txt"
download_file(file, website, folder, "spike_info.txt")
...
plot(MCMC_naive, Param = "epsilon", Gene = 10000)
...
ggplot() +
  geom_point(
    data = data.frame(
      Mu = HVG$Table$Mu[HVG_res == "FALSE FALSE"],
      Epsilon = HVG$Table$Epsilon[HVG_res == "FALSE FALSE"],
      HVG = HVG_res[HVG_res == "FALSE FALSE"]
    ),
...
MCMC_active <- readRDS("MCMC_active.rds")
```
...
plot(MCMC_active, Param = "epsilon", Gene = 10000)
...
# Naive CD4+ T cells
ess.naive <- coda::effectiveSize(coda::mcmc(displayChainBASiCS(MCMC_naive, "mu")))
low_ess_genes.naive <- names(ess.naive[ess.naive < 100])

# Active CD4+ T cells
ess.active <- coda::effectiveSize(coda::mcmc(displayChainBASiCS(MCMC_active, "mu")))
...
genes_select <- !(colnames(displayChainBASiCS(MCMC_naive)) %in% to_exclude)
...
if (!file.exists("cellAnnotation.tsv")) {
