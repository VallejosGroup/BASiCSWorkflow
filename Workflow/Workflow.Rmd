...
To assess whether the chains converged, we will visualize trace plots for some of the parameters. The `mcmc` function of the `coda` package is suited to generate trace plots for each chain. 

```{r droplet-chain-convergence}
plot(mcmc(displayChainBASiCS(PSM.MCMC, Param = "mu")[,120]))
plot(mcmc(displayChainBASiCS(PSM.MCMC, Param = "delta")[,10]))
plot(mcmc(displayChainBASiCS(PSM.MCMC, Param = "epsilon")[,20]))
plot(mcmc(displayChainBASiCS(PSM.MCMC, Param = "s")[,90]))
plot(mcmc(displayChainBASiCS(PSM.MCMC, Param = "nu")[,100]))
plot(mcmc(displayChainBASiCS(PSM.MCMC, Param = "beta")[,2]))
plot(mcmc(displayChainBASiCS(PSM.MCMC, Param = "sigma2")))

plot(mcmc(displayChainBASiCS(SM.MCMC, Param = "mu")[,120]))
plot(mcmc(displayChainBASiCS(SM.MCMC, Param = "delta")[,10]))
plot(mcmc(displayChainBASiCS(SM.MCMC, Param = "epsilon")[,20]))
plot(mcmc(displayChainBASiCS(SM.MCMC, Param = "s")[,90]))
plot(mcmc(displayChainBASiCS(SM.MCMC, Param = "nu")[,100]))
plot(mcmc(displayChainBASiCS(SM.MCMC, Param = "beta")[,2]))
plot(mcmc(displayChainBASiCS(SM.MCMC, Param = "sigma2")))
```
