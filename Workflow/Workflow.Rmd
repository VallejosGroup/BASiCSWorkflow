...
```{r gene-selection}
# Remove genes
sce.naive_active <- sce.naive_active[apply(counts(sce.naive_active), 1, 
                                           function(n){sum(n > 0)}) > 1,]
sce.naive_active <- sce.naive_active[rowMeans(counts(sce.naive_active)) >= 1,]
```
...
sce.naive_active <- sce.naive_active[,sce.naive_active$log10_total_counts_endogenous > 5.8]
```
...
```{r SCE-naive}
sce.naive <- sce.naive_active[,sce.naive_active$Stimulus == "Unstimulated"]
```
...
SpikeInput <- ERCC.count[rownames(sce.naive)[isSpike(sce.naive)],]
...
# Effective sample size
diagPlot(MCMC.naive, Param = "delta")
...
# Effective sample size
diagPlot(MCMC.naive, Param = "epsilon")
...
# Exclude genes
counts.denoised <- counts.denoised[!is.na(displayChainBASiCS(
  MCMC.naive, Param = "epsilon")[1,]),]
...
# Remove genes
mu.naive <- displaySummaryBASiCS(Summary.naive, Param = "mu")[
  !is.na(displaySummaryBASiCS(Summary.naive, Param = "epsilon")[,"median"]),
]
delta.naive <- displaySummaryBASiCS(Summary.naive, Param = "delta")[
  !is.na(displaySummaryBASiCS(Summary.naive, Param = "epsilon")[,"median"]),
]
...
epsilon.naive <- displaySummaryBASiCS(Summary.naive, Param = "epsilon")[
  !is.na(displaySummaryBASiCS(Summary.naive, Param = "epsilon")[,"median"]),
]
...
```{r Data-active}
# Select the ERCC spike-ins of the dataset
SpikeInput <- ERCC.count[rownames(sce.active)[isSpike(sce.active)],]
