...
for(j in 1:nrow(SM.GO)){
  ...
  genes = as.character(genes[genes %in% SM.mean$Symbol])
...
for(j in 1:nrow(PSM.GO)){
  ...
  genes = unique(unlist(mget(allegs, org.Mm.egSYMBOL)))
  genes = as.character(genes[genes %in% PSM.mean$Symbol])
  all_genes[j] <- paste(genes, collapse = ", ")
}
...
```{r gene-lists-mean}
# Highly variable in somitic mesoderm
SM.var <- Droplet.Test$TableResDisp[Droplet.Test$TableResDisp$ResultDiffResDisp == "SM+",]
SM.var <- SM.var[order(SM.var$ResDispDistance, decreasing = FALSE),]
SM.var$Symbol <- genenames[SM.var$GeneName,2]

# Highly variable in pre-somitic mesoderm
PSM.var <- Droplet.Test$TableResDisp[Droplet.Test$TableResDisp$ResultDiffResDisp == "PSM+",]
PSM.var <- PSM.var[order(PSM.var$ResDispDistance, decreasing = TRUE),]
PSM.var$Symbol <- genenames[PSM.var$GeneName,2]
```
...
plotExpression(droplet.sce, features = "ENSMUSG00000036144", x = "cellType", 
               colour_by = "cellType")
plotReducedDim(droplet.sce, use_dimred = "PCA", colour_by = "ENSMUSG00000036144")
```
