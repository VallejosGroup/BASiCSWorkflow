...
```{r gene-selection}
# Remove genese)
ind_expressed <- rowSums(counts(sce.naive_active) > 0) > 1 &
...
clean_colnames <- make.names(colnames(colData(sce.naive_active)))
colnames(colData(sce.naive_active)) <- clean_colnames
...
ind_expressed <- rowSums(counts(sce.naive) > 0) > 1 & 
  rowMeans(counts(sce.naive)) >= 1
...
altexp_present <- altExp(sce.naive, "spike-ins")[spikes_present, ]
altExp(sce.naive, "spike-ins") <- altexp_present
...
metadata(Data.naive)$SpikeInput <- SpikeInput
```
