...
abstract: "Cell-to-cell variability in gene expression is an inherent feature of
  ...
  a droplet-based system, for which spike-in RNA is not available. The latter analysis provides an example, in which differential variability testing 
...
otherwise undetected in bulk experiments [@Stegle2015; @Prakadan2017; 
@Patange2018]. Its applications range from characterising cell types in 
immunity [@Lonnberg2017;  @Villani2017; @Zheng2017] and development 
[@Ibarra-Soria2018; @Wagner2018; @Pijuan-Sala2019] to dissecting the mechanism 
for cell fate commitment events [@Goolam2016; @Ohnishi2014] . 
Within a population of cells, transcriptional heterogeneity can be due a 
mixture of possible causes. 
On the broadest level, this heterogeneity can reflect the presence of cell 
subtypes with distinct expression profiles.
In such cases, clustering strategies can be used to select individual cell 
types and cell states for individual analyses [@Kiselev2019].
More subtle variation within a seemingly homogeneous cell population can be due 
to deterministic or stochastic events, the later being referred to as 'noise' 
[@Elowitz2002]. 
...
components, which is induced by cells residing in different dynamic states 
(cell size, cell cycle, metabolism, intra- and inter-cellular 
signalling)[@Zopf2013; @Iwamoto2016; @Kiviet2014]. 
Intrinsic noise on the other hand arises from stochastic effects on biochemical 
processes such as transcription and translation [@Elowitz2002]. 
It can be modulated by genetic and epigenetic modifications (such as mutations, 
histone modifications, CpG island length and nucleosome 
positioning)[@Eberwine2015; @Faure2017; @Morgan2018] and is usually measured on 
the single-gene level [@Elowitz2002]. 
However, the observed cell-to-cell variability in gene expression or protein 
abundance arise from a combination of intrinsic and extrinsic noise, and 
determnisitic contributions to cell state differences in cell populations.
Therefore, scRNA-Seq only allows to capture the molecular pehnotypic variability
within cell populations, rather than the individual noise components 
[@Patange2018; @Ecker2017a]. 
Here, the molecular phenotype is defined via the absolute abundance of 
intracellular molecules, such as transcripts, proteins, metabolites, etc.
...
external RNA spike-in molecules (such as the ones introduced by the External RNA 
Controls Consortium, ERCCs [@Rna2005]) can be added to the experiment before 
sequencing. 
...
variability than the technical background [@Brennecke2013]. 
...
We will also store information on the spike-in molecules using the `isSpike` 
accessor. 
Therefore, the SCE object stores gene-specific information whether or not a gene 
is of biological (endogeneous) or technical (spike-in) origin.
...
sce.CD4_all <- SingleCellExperiment(
  assays = list(counts = as.matrix(bio.counts)),
...
**TODO: Add explanation that the number of spike-ins need to be consistent across all 
wells and experiments but that the scale does not effect the results.**
...
ERCC.count <- data.frame(
  row.names = ERCC.conc$ERCC.ID,
...
For comparison, this sampler runs for **TODO: XYZ minutes** on a 3.1 GHz Intel Core 
i7 procesor with 16GB RAM.
**Cata can you run this on your machine as comparison?**
For convenience, the MCMC chain can be obtained online at 
[https://jmlab-gitlab.cruk.cam.ac.uk/publications/BASiCSWorkflow2018](https://jmlab-gitlab.cruk.cam.ac.uk/publications/BASiCSWorkflow2018).
...
robust downstream analysis. There are multiple ways of how to visualize and 
test the convergence of the chains *TODO: refs* and the `r CRANpkg("coda")` 
contains diagnostic and plot functions for this task *TODO: ref*.
...
data and correct for biases in mRNA content *TODO: ref Vallejos 2017*.
...
This analysis results in the detection of *265* highly variable genes and *778*
lowly variable genes.
...
ggplot() +
  geom_point(
    data = data.frame(
      Mu = HVG$Table$Mu[HVG.res == "FALSE FALSE"],
...
`displayChainBASiCS(MCMC.naive, Param = "epsilon")` to contain `NA`.
...
`BASiCS` and `scran`.
**TODO: Add density to these plots**
...
ggplot(data.frame(
  epsilon = epsilon.naive[rownames(counts.denoised), "median"],
  DM = DM.naive
)) +
...
[@Morgan2018,@Faure2018] or transcriptional dynamics [@Antolovic2017] to 
...
[@Martinez-jimnenez2017]. Naive CD4^+^ T cells were activated for 3 hours 
...
above **TODO: link to filtering section**. Therefore, we can directly select the 
activated CD4^+^ T cells from the `sce.naive_active` object.
...
Similar to the procedure described above *TODO: link to naive section*, 
we will use 
...
BASiCS takes this `SingleCellExperiment` object and runs the MCMC sampler over 
40,000 iterations. 

```{r MCMC-active, echo=FALSE, eval=FALSE}
MCMC.active <- BASiCS_MCMC(
  Data = Data.active,
  N = 40000,
  Thin = 20,
  Burn = 20000,
...
This sampler runs for *98 minutes* on a 1.4 GHz Intel Core i5 procesor with 
4GB RAM and produces a `BASiCS_Chain` data object.
This sampler runs for *TODO: XYZ minutes* on a 3.1 GHz Intel Core i7 procesor with 
16GB RAM and produces a `BASiCS_Chain` data object.
...
To perform robust differential mean expression testing, we will select those
genes with small effective sample size for mean expression parameters and 
exclude them from testing.
As explained above *TODO: Add link pointing at the first convergence section*, 
this 
can be done using functions from the `CRANpkg(coda)` package.
We will select genes with low ESS from the `BASiCS_Chain` objects for naive 
...
In total, we exlude 1936 genes from testing due to low effective sample size 
...
with strong changes in mean expression.
...
# Low ESS for mu
ess.naive.mu <- coda::effectiveSize(
  coda::mcmc(displayChainBASiCS(MCMC.naive, "mu"))
)
ind_ess.naive.mu <- ess.naive.mu > 100
ess.active.mu <- coda::effectiveSize(
  coda::mcmc(displayChainBASiCS(MCMC.active, "mu"))
)
ind_ess.active.mu <- ess.naive.mu > 100

# Low ESS for delta
ess.naive.epsilon <- coda::effectiveSize(
  coda::mcmc(displayChainBASiCS(MCMC.naive, "epsilon"))
)
ind_ess.naive.epsilon <- ess.naive.mu > 100
ess.active.epsilon <- coda::effectiveSize(
  coda::mcmc(displayChainBASiCS(MCMC.active, "epsilon"))
)
...
With the development of droplet-based scRNA-Seq [@Klein2015, @Makosco2015] 
...
variation through replication [@Carrol1998].
...
```{r, eval = FALSE}
Droplet.Test.logFC0 <- BASiCS_TestDE(
  PSM.MCMC,
  SM.MCMC,
  EpsilonM = 0,
  GroupLabel1 = "PSM",
...
not_excluded <- Droplet.Test.logFC0$TableDisp$ResultDiffDisp != "ExcludedFromTesting"
for.plot <- Droplet.Test.logFC0$TableDisp[not_excluded, c("Disp1", "Disp2")]
...
Droplet.Test <- BASiCS_TestDE(PSM.MCMC, SM.MCMC,
  EpsilonM = 1,
...
pheatmap(for.heatmap,
  ...
  cellheight = 8, fontsize = 7, annotation_col = data.frame(
    row.names = colnames(for.heatmap),
    cellType = sub("_.*", "", colnames(for.heatmap))
  ), scale = "row"
)
```
...
gene.groups <- data.frame(
  Genename = Droplet.Test$TableResDisp$GeneName,
