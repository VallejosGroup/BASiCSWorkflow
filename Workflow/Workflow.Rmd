...
In the first case, we use scRNA-Seq data of mouse embryonic stem cells (mESCs) [@Kolodziejczyk2015] which were processed using the C1 Single-Cell Auto Prep System (Fluidigm®). 
...
The raw counts of the full dataset can be obtain from the [ESpresso](https://www.ebi.ac.uk/teichmann-srv/espresso/) database.
The column names contain the necessary meta-information regarding the culture consition and batch.
The row names of the matrix store the individual gene names. 
...
```{r mESC-data}
if(!file.exists("mESC_raw.csv")){
  download.file("https://www.ebi.ac.uk/teichmann-srv/espresso/static/counttable_es.csv", 
                destfile = "mESC_raw.csv")
}
mESC.raw <- read.table("mESC_raw.csv", header = TRUE)

# Show row and column names
head(colnames(mESC.raw))
head(rownames(mESC.raw))

# ERCC spike-in genes
rownames(mESC.raw)[grepl("ERCC", rownames(mESC.raw))]
```
