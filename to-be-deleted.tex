...
In \emph{\href{https://bioconductor.org/packages/3.11/scran}{scran}}, the squared coefficient of variation (CV\textsuperscript{2}) is used
as a proxy for transcriptional variability within a population of cells
Instead, \emph{\href{https://bioconductor.org/packages/3.11/BASiCS}{BASiCS}} uses gene-specific over-dispersion
parameters \(\delta_i\) for this purpose.
As seen in Fig .. these estimates are highly correlated.
Both variability measures are confounded by differences in mean expression,
e.g.~highly expressed genes tend to exhibit lower variability according to these
metrics. For example, Figure \ref{fig:dispersion-vs-mean}A displays
\emph{\href{https://bioconductor.org/packages/3.11/BASiCS}{BASiCS}} posterior estimates for over-dispersion parameters
inferred for the naive CD4\textsuperscript{+} T cells and how these relate to mean expression
estimates (the \texttt{BASiCS\_ShowFit} function can be used to generate such a plot,
but we will generate it manually to demonstrate how users can extract this
information from a \texttt{BASiCS\_MCMC} object.
A similar behaviour is observed for \emph{\href{https://bioconductor.org/packages/3.11/scran}{scran}} estimates (see Figure \ref{fig:dispersion-vs-mean}B).
...
Recently \citep{Eling2018}, we extended \emph{\href{https://bioconductor.org/packages/3.11/BASiCS}{BASiCS}} to explicitly model
...
\ref{fig:dispersion-vs-mean}. This results in the \(\epsilon\) parameter, a measure of variability that is not confounded by mean expression. This \emph{residual}
...
Similarly, Kolodziejczyk \emph{et al.} used the distance to a rolling
median (DM) along the mean-variability trend to correct for this confounding
factor \citep{Kolodziejczyk2015cell}. Here, we highlight how to obtain the residual
variability estimates using \emph{\href{https://bioconductor.org/packages/3.11/BASiCS}{BASiCS}} and \emph{\href{https://bioconductor.org/packages/3.11/scran}{scran}}.
...
least 2 cells. These genes have missing (\texttt{NA}) values for the residual
over-dispersion parameter, \(\epsilon\). These values can be obtained using the
function \texttt{displayChainBASiCS}. We will use \emph{\href{https://CRAN.R-project.org/package=ggpointdensity}{ggpointdensity}}
...
show that variance and CV\textsuperscript{2} are both highly correlated with the
over-dispersion estimates from
\emph{\href{https://bioconductor.org/packages/3.11/BASiCS}{BASiCS}}. Again, for this comparison,
...
over-dispersion parameters estimated by \emph{\href{https://bioconductor.org/packages/3.11/BASiCS}{BASiCS}} are strongly
...
\citep{Morgan2018, Faure2017} or transcriptional dynamics \citep{Antolovic2017} with
gene expression variability.
While the DM is calculated as a point estimate, \emph{\href{https://bioconductor.org/packages/3.11/BASiCS}{BASiCS}} stores
each posterior sample within the \texttt{BASiCS\_Chain} object.
They can be accessed using the \texttt{displayChain} function. This function
