...
This analysis results in the detection of \emph{247}
highly variable genes and \emph{733}
...
\NormalTok{HVG_table <-}\StringTok{ }\NormalTok{HVG_table[HVG_table}\OperatorTok{$}\NormalTok{Mu }\OperatorTok{>}\StringTok{ }\DecValTok{1}\NormalTok{,]}
\NormalTok{LVG_table <-}\StringTok{ }\NormalTok{LVG_table[LVG_table}\OperatorTok{$}\NormalTok{Mu }\OperatorTok{>}\StringTok{ }\DecValTok{1}\NormalTok{,]}

\KeywordTok{library}\NormalTok{(reshape2)}
\NormalTok{topHVG <-}\StringTok{ }\KeywordTok{log10}\NormalTok{(}\KeywordTok{t}\NormalTok{(DC_naive[HVG_table}\OperatorTok{$}\NormalTok{GeneName[}\DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{], ]) }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{)}
\NormalTok{topLVG <-}\StringTok{ }\KeywordTok{log10}\NormalTok{(}\KeywordTok{t}\NormalTok{(DC_naive[LVG_table}\OperatorTok{$}\NormalTok{GeneName[}\DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{], ]) }\OperatorTok{+}\StringTok{ }\DecValTok{1}\NormalTok{)}
...
\StringTok{  }\KeywordTok{geom_violin}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{coord_flip}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{ylim}\NormalTok{(}\OperatorTok{-}\FloatTok{0.01}\NormalTok{,}\FloatTok{3.2}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_jitter}\NormalTok{(}\DataTypeTok{position=}\KeywordTok{position_jitter}\NormalTok{(}\FloatTok{0.3}\NormalTok{))}
...
\StringTok{  }\KeywordTok{geom_violin}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{coord_flip}\NormalTok{() }\OperatorTok{+}\StringTok{ }\KeywordTok{ylim}\NormalTok{(}\OperatorTok{-}\FloatTok{0.01}\NormalTok{,}\FloatTok{3.2}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_jitter}\NormalTok{(}\DataTypeTok{position=}\KeywordTok{position_jitter}\NormalTok{(}\FloatTok{0.3}\NormalTok{))}
\KeywordTok{plot_grid}\NormalTok{(plot_hvg, plot_lvg)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics{figure/Denoised_naive-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{is_hvg <-}\StringTok{ }\NormalTok{var_out}\OperatorTok{$}\NormalTok{ratio }\OperatorTok{>}\StringTok{ }\DecValTok{2} \OperatorTok{&}\StringTok{ }\NormalTok{var_out}\OperatorTok{$}\NormalTok{FDR }\OperatorTok{<}\StringTok{ }\FloatTok{0.05}
\NormalTok{ind_hvg <-}\StringTok{ }\KeywordTok{which}\NormalTok{(is_hvg)}
\NormalTok{hvg_out <-}\StringTok{ }\NormalTok{var_out[ind_hvg, ]}
\NormalTok{hvg_out <-}\StringTok{ }\NormalTok{hvg_out[}\KeywordTok{order}\NormalTok{(hvg_out}\OperatorTok{$}\NormalTok{FDR), ]}
\KeywordTok{nrow}\NormalTok{(hvg_out)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Compare BASiCS and scran results}
\NormalTok{plot_data <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
  \DataTypeTok{Mu =}\NormalTok{ HVG_table}\OperatorTok{$}\NormalTok{Mu,}
  \DataTypeTok{Epsilon =}\NormalTok{ HVG_table}\OperatorTok{$}\NormalTok{Epsilon,}
  \DataTypeTok{BASiCS_HVG =}\NormalTok{ HVG_table}\OperatorTok{$}\NormalTok{HVG,}
  \DataTypeTok{scran_HVG =}\NormalTok{ is_hvg}
\NormalTok{)}
\NormalTok{plot_data <-}\StringTok{ }\NormalTok{plot_data[}\KeywordTok{complete.cases}\NormalTok{(plot_data), ]}
\NormalTok{plot_data <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(plot_data, }
  \DataTypeTok{BASiCS =} \KeywordTok{ifelse}\NormalTok{(plot_data}\OperatorTok{$}\NormalTok{BASiCS_HVG, }\StringTok{"HVG"}\NormalTok{, }\StringTok{"Not HVG"}\NormalTok{),}
  \DataTypeTok{scran =} \KeywordTok{ifelse}\NormalTok{(plot_data}\OperatorTok{$}\NormalTok{scran_HVG, }\StringTok{"HVG"}\NormalTok{, }\StringTok{"Not HVG"}\NormalTok{)}
\NormalTok{)}

\NormalTok{ord <-}\StringTok{ }\KeywordTok{order}\NormalTok{(plot_data}\OperatorTok{$}\NormalTok{scran, plot_data}\OperatorTok{$}\NormalTok{BASiCS, }\DataTypeTok{decreasing =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{plot_data <-}\StringTok{ }\NormalTok{plot_data[ord, ]}
\NormalTok{plot_data}\OperatorTok{$}\NormalTok{Status <-}\StringTok{ }\KeywordTok{paste0}\NormalTok{(}
  \StringTok{"BASiCS: "}\NormalTok{, plot_data}\OperatorTok{$}\NormalTok{BASiCS, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{,}
  \StringTok{"scran: "}\NormalTok{, plot_data}\OperatorTok{$}\NormalTok{scran, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\KeywordTok{ggplot}\NormalTok{(plot_data) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_point}\NormalTok{(}
    \KeywordTok{aes}\NormalTok{(}\KeywordTok{log}\NormalTok{(Mu), Epsilon, }\DataTypeTok{colour =}\NormalTok{ Status),}
...
{\centering \includegraphics{figure/scran-HVG-1} 

}

\caption{Epsilon (residual over-dispersion) is plotted against log(mu) (log mean expression). Colour indicates the results of scran and BASiCS HVG tests.}\label{fig:scran-HVG}
\end{figure}

\emph{39}
of the \emph{1134} HVGs of scran are among the
\emph{222} HVGs identified by \emph{\href{https://bioconductor.org/packages/3.11/BASiCS}{BASiCS}}.
