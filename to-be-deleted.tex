...
\NormalTok{ERCC_count <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
  \DataTypeTok{row.names =}\NormalTok{ ERCC_conc}\OperatorTok{$}\NormalTok{ERCC.ID,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ind_stimulated <-}\StringTok{ }\NormalTok{sce_naive_active}\OperatorTok{$}\NormalTok{Stimulus }\OperatorTok{==}\StringTok{ "Unstimulated"}
\NormalTok{sce_naive <-}\StringTok{ }\NormalTok{sce_naive_active[, ind_stimulated]}
\NormalTok{ind_expressed <-}\StringTok{ }\KeywordTok{rowSums}\NormalTok{(}\KeywordTok{counts}\NormalTok{(sce_naive) }\OperatorTok{>}\StringTok{ }\DecValTok{0}\NormalTok{) }\OperatorTok{>}\StringTok{ }\DecValTok{1} \OperatorTok{&}\StringTok{ }
\StringTok{  }\KeywordTok{rowMeans}\NormalTok{(}\KeywordTok{counts}\NormalTok{(sce_naive)) }\OperatorTok{>=}\StringTok{ }\DecValTok{1}
\NormalTok{sce_naive <-}\StringTok{ }\NormalTok{sce_naive[ind_expressed, ]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Here is the first time that we use BASiCS}

\CommentTok{# Select the ERCC spike-ins of the dataset}
\NormalTok{counts <-}\StringTok{ }\KeywordTok{counts}\NormalTok{(sce_naive)}
\NormalTok{spikes <-}\StringTok{ }\KeywordTok{assay}\NormalTok{(}\KeywordTok{altExp}\NormalTok{(sce_naive, }\StringTok{"spike-ins"}\NormalTok{))}
\NormalTok{spikes_present <-}\StringTok{ }\KeywordTok{rowSums}\NormalTok{(spikes) }\OperatorTok{!=}\StringTok{ }\DecValTok{0}
\CommentTok{## Remove spike-ins that are not present from matrix and SCE object}
\NormalTok{spikes <-}\StringTok{ }\NormalTok{spikes[spikes_present, ]}
\NormalTok{altexp_present <-}\StringTok{ }\KeywordTok{altExp}\NormalTok{(sce_naive, }\StringTok{"spike-ins"}\NormalTok{)[spikes_present, ]}
\KeywordTok{altExp}\NormalTok{(sce_naive, }\StringTok{"spike-ins"}\NormalTok{) <-}\StringTok{ }\NormalTok{altexp_present}
\NormalTok{ind_spike <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{rep}\NormalTok{(}\OtherTok{FALSE}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(counts)), }\KeywordTok{rep}\NormalTok{(}\OtherTok{TRUE}\NormalTok{, }\KeywordTok{nrow}\NormalTok{(spikes)))}
\NormalTok{spike_input <-}\StringTok{ }\NormalTok{ERCC_count[}\KeywordTok{rownames}\NormalTok{(spikes), ]}

\CommentTok{# Generate the SingleCellExperiment object}
\NormalTok{data_naive <-}\StringTok{ }\KeywordTok{newBASiCS_Data}\NormalTok{(}
  \DataTypeTok{Counts =} \KeywordTok{rbind}\NormalTok{(counts, spikes),}
  \DataTypeTok{Tech =}\NormalTok{ ind_spike,}
  \DataTypeTok{SpikeInfo =}\NormalTok{ spike_input,}
  \DataTypeTok{BatchInfo =}\NormalTok{ sce_naive}\OperatorTok{$}\NormalTok{Individuals}
\NormalTok{)}
\NormalTok{data_naive}
\end{Highlighting}
\end{Shaded}
...
original \texttt{SingleCellExperiment} object can be extended by simply specifying a
\texttt{BatchInfo} slot in the \texttt{colData} object and by adding the \texttt{SpikeInfo} object
to the \texttt{metadata} slot.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{data_naive <-}\StringTok{ }\NormalTok{sce_naive}
\KeywordTok{colData}\NormalTok{(data_naive)}\OperatorTok{$}\NormalTok{BatchInfo <-}\StringTok{ }\KeywordTok{colData}\NormalTok{(sce_naive)}\OperatorTok{$}\NormalTok{Individuals}
\KeywordTok{metadata}\NormalTok{(data_naive)}\OperatorTok{$}\NormalTok{SpikeInput <-}\StringTok{ }\NormalTok{spike_input}
\end{Highlighting}
\end{Shaded}
